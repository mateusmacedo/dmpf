...
a saída é um módulo Go em `libs/backend/go/<name>`, com um package por bloco
(`domain`, `ports`, `application`, `provider`, `app`), um contrato e um OpenAPI
que passam pelos mesmos gates de qualquer módulo do workspace (ADR-045). Não existe DSL: a spec é a única fonte
...
Copie a forma, não o conteúdo: `domain/example/orders` para o agregado

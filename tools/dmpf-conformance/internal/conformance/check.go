...
func (r Report) Resumo() string {
	if len(r.NaoVerificado) > 0 {
		return fmt.Sprintf("conformance: REPROVADO com %d diagnóstico(s) e %d condição(oes) não verificada(s)",
			len(r.Diagnostics), len(r.NaoVerificado))
	}
	if len(r.Diagnostics) == 0 {
		return "conformance: conforme"
	}
	if r.PhaseHalted != "" {
		return fmt.Sprintf("conformance: REPROVADO com %d diagnóstico(s); parou em %s",
			len(r.Diagnostics), r.PhaseHalted)
	}
	return fmt.Sprintf("conformance: REPROVADO com %d diagnóstico(s)", len(r.Diagnostics))
}

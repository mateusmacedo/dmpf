...
# restringe os blocos gerados; DMPF_GENERATOR_CHECK_SIMULAR_SEM_INSTRUCAO=1
# filtra a instrução do baseline da saída do generator (vetor negativo da fase
# `self-test`).
set -uo pipefail
...
uso() {
  cat <<'FIM'
uso: tools/dmpf-generator-check.sh --phase structural|integration|self-test
  ...
  integration  ainda não implementada (tarefa 3.3 do plano)
  self-test    ainda não implementada (tarefa 3.4 do plano)
FIM
}
...
ancorar_node_modules() {
  ...
  NM_ANCORA="$(readlink -m "$ROOT/node_modules/nx")"
  case "$NM_ANCORA" in
    "$ROOT"/*) ;;
    *) falha "node_modules/nx da raiz resolve para fora de $ROOT ($NM_ANCORA); rode 'pnpm install' na raiz antes do gate" ;;
  esac
  [ -f "$NM_ANCORA/package.json" ] \
    || falha "node_modules/nx da raiz está quebrado ($NM_ANCORA sem package.json); rode 'pnpm install' na raiz antes do gate"
}

conferir_node_modules() {
  local agora
  agora="$(readlink -m "$ROOT/node_modules/nx")"
  [ "$agora" = "$NM_ANCORA" ] \
    || falha "o node_modules da raiz foi reescrito durante o gate ($NM_ANCORA -> $agora): o pnpm instalou por dentro do symlink do worktree; rode 'pnpm install' na raiz para recuperar"
}
...
case "$FASE" in
  ...
  integration | self-test)
    echo "FALHA: fase ainda não implementada: $FASE" >&2
    exit 1
    ;;
  "")

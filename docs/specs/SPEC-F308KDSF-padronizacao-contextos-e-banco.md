...
- [x] **[P0] Nomear as variáveis de ambiente sem o prefixo `DMPF_`**: a configuração das apps e das libs e tudo o que a consome (compose, Kubernetes, CI, docs vivas) usam o nome expressivo sem vínculo com a marca (`PG_DSN`, `GRPC_ADDR`, `KAFKA_ORDERS_TOPIC`). O tooling de `tools/` e dos kits de teste mantém o prefixo (`DMPF_GENERATOR_CHECK_*`, `DMPF_HARNESS_CHECK_AGENT_CMD`, `DMPF_TESTKIT_*`, `DMPF_TB_*`, `DMPF_EVIDENCE_DIR`). ADRs aceitos, specs históricas e a evidência de release publicada não são reescritos.

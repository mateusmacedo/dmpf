...
- [ ] **[P0] Grupo único para as libs do kernel**: declarar em `nx.json` um grupo com os 14 projetos de `libs/backend/go/*`, `releaseTag.pattern` `libs/backend/go/{projectName}/v{version}`, as version actions padrão do `@nx/js`, `currentVersionResolver: "git-tag"`, `fallbackCurrentVersionResolver: "disk"` e `updateDependents: "always"`. O padrão com `{projectName}` é possível porque o ADR-045 alinhou nome e diretório.

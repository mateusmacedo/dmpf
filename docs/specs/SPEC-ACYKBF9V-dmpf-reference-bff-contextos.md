...
Substitui o composition root único `apps/backend/dmpf-reference` por três:
`dmpf-reference-bff-go`, a única borda REST/JSON pública, e
`dmpf-reference-orders-go` e `dmpf-reference-reservations-go`, cada um com
...
- **Problema**: `dmpf-reference` mistura a borda HTTP de `orders` com o consumo
...
- **Links relevantes**:
  ...
  - `SPEC-EAGAXQN1` — `dmpfgrpc.Dial`, `NewServer`, `Admission`, `HTTPStatus`; `dmpfhttp.Route`
...
| Stubs gRPC no bloco `contract` de `dmpf-contracts` | `capabilityPolicy` permite só `pure` e `wire.codec` ao `contract` (`dmpf-conformance/internal/rule/capability.go:48`); `google.golang.org/grpc` é `io.network` (`dmpf-provider-grpc/dmpf-units.json`); `buf.gen.yaml` só tem `protoc-gen-go` | O `.proto` de `service` gera **só mensagens e descriptor** pelo `protoc-gen-go` já pinado. O binding gRPC (`grpc.ServiceDesc` no servidor, `ClientConn.Invoke` no cliente) é escrito no bloco `app` de cada composition root, com o nome do método derivado do descriptor gerado. Sem `protoc-gen-go-grpc`, sem mudança em `buf.gen.yaml` nem em `gate_pins` |
| Relay não compartilhado; cada contexto drena só o próprio destino | `OutboxStore.Claim(ctx, claimID, limit, lease)` não filtra por destino (`dmpf-provider-postgres/claim.go:120`) | **Banco por contexto**: `orders` e `reservations` têm `DMPF_PG_DSN` próprios, e cada relay drena a outbox do próprio banco. Kernel inalterado |
...
| Fechar `metadata = "{}"` é condição de drenagem | `encodeMetadata` grava `"{}"` com contexto zero (`outbox.go:96-98`); o relay exige as três chaves (`relay/record.go:67-88`) | O interceptor de servidor gRPC do contexto grava `dmpfports.WithMessageContext` antes de chamar o caso de uso; `MessageContextFor` só preenche `causationid` quando vazio |
...
<constraints>
- [P0] NUNCA importar pacote de `domain` ou `application` dos exemplos no BFF: ele declara `bounded_context` `dmpf-reference-bff` e só alcança a superfície `contract` e o shared kernel; qualquer outra aresta é `DMPF-D002`.
...
- [P1] Todo span novo usa só atributos de `dmpf-observability/tracing` (`TRC-15`); nenhum span em `domain` ou `port` (`TRC-16`).
</constraints>
...
- [ ] **[P0] Geração e manifesto**: o `include` de `dmpf-contracts/gen` ganha `gen/go/company/orders/service/v1` e `gen/go/company/reservations/service/v1` antes de `buf generate`; o gerado entra versionado; `buf-lint`, `buf-pins`, `buf-generate-check` e `NX_BASE=develop buf-breaking` passam sem mudança de pin nem de gate (mudança aditiva).
...
**Contextos (`dmpf-reference-orders-go`, `dmpf-reference-reservations-go`)**

- [ ] **[P0] Módulos e projetos**: `apps/backend/dmpf-reference-orders-go` e `apps/backend/dmpf-reference-reservations-go`, diretório igual ao nome do projeto Nx; `go.mod` sem `require` de irmão; entrada no `go.work`; tags `type:app`, `scope:backend`, `stack:go`, `layer:apps`; targets `fmt-check`, `vet`, `build`, `test-race` (`cache: false`), `govulncheck` e `serve-<papel>`; `package.json` `private: true`; `Dockerfile` por app; `dmpf-units.json` com unidade `app`, `bounded_context` `dmpf-kernel`, `include` por import path exato e `external` só com o que os providers cabeados exigem.
- [ ] **[P0] Papel `api` gRPC**: `dmpfgrpc.NewServer` com `Services` declarados e health `SERVING` só após o `Migrate` opcional e o pool prontos; interceptors de servidor na ordem span de servidor → `dmpfgrpc.Admission` → `deadline.Require` (sem deadline: `INVALID_ARGUMENT` antes do handler) → contexto de mensagem → handler. O handler decodifica o Protobuf, chama o caso de uso e mapeia `ErrNotFound` → `NOT_FOUND`, conflito de versão → `ABORTED`, prazo → `DEADLINE_EXCEEDED`, demais → `INTERNAL` sem mensagem interna. `orders` serve `AddItem`, `PlaceOrder` e `FindOrder`; `reservations` serve `Reserve`, `Cancel` e `FindReservation`.
...
**BFF (`dmpf-reference-bff-go`)**

- [ ] **[P0] Módulo e projeto**: `apps/backend/dmpf-reference-bff-go`, mesmas convenções dos contextos; unidade `dmpf-reference-bff/app`, bloco `app`, `bounded_context` `dmpf-reference-bff`; `external` sem `pgx` nem `franz-go`. Recebe os targets de infraestrutura hoje em `dmpf-reference-go` (`infra-up`, `observability-up`, `infra-down`, `infra-budget`, `k8s-render`).
- [ ] **[P0] Borda REST**: um processo (`serve`); rotas `POST /orders/{id}/items`, `POST /orders/{id}/place`, `GET /orders/{id}`, `GET /reservations/{order_id}`, `POST /reservations/{order_id}/reserve`, `POST /reservations/{order_id}/cancel`, cada uma um `dmpfhttp.Route` validado na construção com `ContractRef` para o OpenAPI do contexto (`RST-04`); middleware na ordem `Admission` (`RES-17`) → span de servidor e contexto (`TRC-02`) → prazo da requisição derivado do `Budget` da rota → `Idempotency-Key` nos POST (400 sem) → handler; CORS opcional por `DMPF_CORS_ORIGINS`; serve os dois OpenAPI em `/openapi/orders/v1/openapi.yaml` e `/openapi/reservations/v1/openapi.yaml`.
- [ ] **[P0] Cliente gRPC**: dois `dmpfgrpc.Dial` (`DMPF_ORDERS_GRPC_TARGET`, `DMPF_RESERVATIONS_GRPC_TARGET`, obrigatórias; transporte por `DMPF_GRPC_INSECURE=true` só em desenvolvimento ou `DMPF_GRPC_CA_FILE` com `DMPF_GRPC_SERVER_NAME` opcional, e sem nenhum dos dois → exit 2) com `MethodPolicy` por método — `FindOrder` e `FindReservation` idempotentes com `UNAVAILABLE` retentável, os demais sem retry — e `Budget` com folga menor que a da rota (`GRP-16`, `GRP-17`). JSON ↔ Protobuf no handler; `Rejection` → 422 com `{code, message}`; status gRPC → HTTP pela tabela de `dmpfgrpc.HTTPStatus`. O breaker de cada dependência conta como falha só indisponibilidade (`UNAVAILABLE`, `DEADLINE_EXCEEDED`, `RESOURCE_EXHAUSTED`, `INTERNAL`, `UNKNOWN`, `DATA_LOSS` e erro de transporte): `NOT_FOUND` e os demais desfechos de negócio não o abrem (`RES-10`, `RES-12`). A classificação vive no kernel — classificador de falha no `resilience.Breaker`, repassado por `dmpf-transport/compose` e aplicado por padrão no `dmpf-provider-grpc` —, sem contorno no BFF.
...
- [ ] **[P0] Spans**: BFF com span HTTP de servidor e o span de cliente de `dmpfgrpc`; contexto com span gRPC de servidor filho do `traceparent` recebido, span de caso de uso distinto (`TRC-05`) e span de banco por `pgx.QueryTracer` no pool, sem SQL nem argumentos; erro sempre amostrado (`TRC-14`).
...
- [ ] **[P1] Painéis**: `infra/observability/grafana/dashboards/dmpf-reference.json` passa a filtrar pelos três serviços.

**Topologia, remoção e documentação**

- [ ] **[P0] Infra local**: `infra/local/compose/dmpf-reference.yml` sobe seis serviços (`dmpf-bff`, `dmpf-orders-api`, `dmpf-orders-relay`, `dmpf-reservations-api`, `dmpf-reservations-relay`, `dmpf-reservations-consumer`) no profile `dmpf`, só o BFF publicado no host, `DMPF_MIGRATE=true` só nos `api`; o Postgres cria os bancos `dmpf_orders` e `dmpf_reservations` na inicialização; o Swagger UI aponta para os dois OpenAPI do BFF; `infra-budget` continua aprovando.
- [ ] **[P0] Kubernetes**: `infra/k8s/base/dmpf-reference-{bff,orders,reservations}` substituem `base/dmpf-reference`; `NetworkPolicy` só admite gRPC nos `api` vindo do BFF; overlays `dev` e `hmg` com imagens e `Secret` por app; `k8s-render` passa.
- [ ] **[P0] Remoção**: `apps/backend/dmpf-reference`, a entrada do `go.work` e `infra/k8s/base/dmpf-reference` saem por `trash`; a unidade `dmpf-kernel/reference-app` sai do baseline no commit de classificação; `dmpf-testkit/project.json` troca o `dependsOn` para os novos projetos; o subject `reference` do `dmpf-evidence` passa a listar os três módulos. `bom/dmpf/0.1.0.json` e `bom/evidence/0.1.0/` ficam intactos (release histórica).
- [ ] **[P0] Classificação**: manifestos novos, `include` de `dmpf-contracts/gen` e `units-baseline.json` regravado em commit próprio, sem código (`DMPF-T001`, `DMPF-T002`); a aprovação por revisor distinto do autor é registrada como pendência de revisão.
- [ ] **[P1] ADR-044 e documentos**: ADR-044 registra a topologia, o binding gRPC no bloco `app`, o banco por contexto e as alternativas descartadas, sem alterar as decisões do ADR-041; `docs/adr/README.md`, `AGENTS.md` (árvore, Apps, tabela `layer:*`, Comandos), `infra/README.md`, os README dos três apps, `libs/backend/go/dmpf-app/{README.md,doc.go}`, `libs/backend/go/dmpf-testkit/README.md`, `docs/guides/{dmpf-arch-review,dmpf-composicao,dmpf-implementation}.md` e `.agents/skills/dmpf-bounded-context/references/{golden-path,armadilhas}.md` apontam para os novos apps. Specs e ADRs históricos não são editados.

### Não-funcionais

- [ ] Conformidade: `dmpf-conformance` sem diagnóstico; `tools/dmpf-gate-check.sh` e `tools/dmpf-cell-check.sh` verdes; nenhuma exceção nova de manifesto.
...
| --- | --- | --- |
| `domain` | [x] | `dmpf-domain/example/reservations`: `Canceled`, `Cancel`, `ReservationCancelled`, códigos |
| `application` | [x] | `dmpf-application/example/reservations`: `Reserve`, `Cancel`, `FindReservation`, `Reader` |
| `port` | [ ] | — |
| `contract` | [x] | protos de `service` e `ReservationCancelled`, gerado, fixtures, OpenAPI |
| `provider` | [x] | `dmpf-provider-postgres/example/reservations`: mapper e reader |
| `app` | [x] | três composition roots novos; `dmpf-reference` removido |
...
libs/backend/go/dmpf-contracts/gen/go/company/{orders,reservations}/...     — GERADO
libs/backend/go/dmpf-contracts/golden/reservation_cancelled_fixture_test.go — NOVO
libs/backend/go/dmpf-contracts/dmpf-units.json                              — MODIFICAR (commit de classificação)
libs/backend/go/dmpf-domain/example/reservations/{messages,reserve,rejections}.go, cancel.go e testes
libs/backend/go/dmpf-application/example/reservations/{service,reserve,cancel,find_reservation}.go e testes
libs/backend/go/dmpf-provider-postgres/example/reservations/{mapper,reader}.go e testes
apps/backend/dmpf-reference-orders-go/                                      — NOVO
  ...
  cmd/dmpf-reference-orders/main.go
apps/backend/dmpf-reference-reservations-go/                                — NOVO (mesma forma + sink.go)
apps/backend/dmpf-reference-bff-go/                                         — NOVO
  ...
  cmd/dmpf-reference-bff/main.go
  e2e_test.go, harness_test.go                                              — integration: seis processos
apps/backend/dmpf-reference/                                                — REMOVER
infra/local/compose/{dmpf-reference,postgres}.yml, infra/local/.env.example — MODIFICAR
infra/k8s/base/dmpf-reference-{bff,orders,reservations}/, overlays/{dev,hmg} — NOVO/MODIFICAR; base/dmpf-reference REMOVER
infra/observability/grafana/dashboards/dmpf-reference.json                  — MODIFICAR
libs/backend/go/dmpf-testkit/{project.json, cmd/dmpf-evidence/main.go}      — MODIFICAR
...
flowchart LR
  ...
  subgraph bffApp [dmpf-reference-bff-go]
    BFF[REST OpenAPI]
  end
  subgraph ordersApp [dmpf-reference-orders-go]
    OrdersAPI[api gRPC]
    OrdersRelay[relay]
    OrdersDB[(dmpf_orders)]
  end
  subgraph reservationsApp [dmpf-reference-reservations-go]
    ResAPI[api gRPC]
    ResRelay[relay]
...
| --- | --- | --- |
| C2, ADR-015, capability do `contract` | `dmpf-conformance` sobre os três apps e `dmpf-contracts` | CI, Gates DMPF |
...
- **Binding gRPC no bloco `app` com mensagens geradas** porque o bloco
  ...
  binding dentro de cada app (código gerado fora de `dmpf-contracts`, contra
...
- [ ] A cadeia de gates fecha verde, nesta ordem: `dmpf-contracts-go:buf-lint`, `buf-pins`, `buf-generate-check`, `NX_BASE=develop buf-breaking`; `tools/dmpf-gate-check.sh`; `tools/dmpf-cell-check.sh`; `dmpf-conformance` com baseline atualizado; `fmt-check`, `vet`, `test-race`, `build` e `govulncheck` nos projetos Go afetados; `k8s-render`, `infra-budget`, `biome ci`, `adr-verify`.
- [ ] `pnpm nx show projects --projects=tag:layer:apps` lista `dmpf-app-go`, `bookings-app-go` e os três novos; `dmpf-reference-go` não existe mais.
...
- [ ] Nenhum arquivo do BFF importa `dmpf-domain`, `dmpf-application` nem os pacotes dos contextos; nenhum pacote gerado importa `google.golang.org/grpc`.
- [ ] ADR-044 criado; `AGENTS.md` e os guias apontam para os três apps; nenhuma referência viva a `apps/backend/dmpf-reference/` fora de specs, ADRs e BOM históricos.

### Cenários de teste

**Domínio (`dmpf-domain/example/reservations`)**
...
**Fim a fim (`dmpf-reference-bff-go/e2e_test.go`, integration)**

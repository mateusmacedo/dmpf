...
| `CTX-03` | import-verifiable + structurally reviewable | diagnóstico reusado de `RFC §10.3` + inspeção: contexto obtido do portador de escopo da requisição; `domain` recebe valores extraídos | `application service` obtém o contexto do portador; UPR não o recebe (FND-03 `FRT-03`) | `domain`/UPR recebe o contexto → aresta proibida | §9 (`FIT`); `RFC §10.3`; FND-07 §3.1 |
| `CTX-04` | structurally reviewable | critério: contexto imutável após montado; derivar sub-contexto é montar outro | — | — | FND-07 §3.1 |
| `CTX-05` | import-verifiable + structurally reviewable | diagnóstico reusado de `RFC §10.3` + inspeção: o portador (`context.Context`, `AsyncLocalStorage`) nasce e morre com a requisição e é a fonte única | portador de escopo da requisição, sem segunda fonte para o mesmo campo; consumidor sem contexto nega | portador que sobrevive à requisição (variável de módulo, singleton) → aresta/diagnóstico | §9 (`FIT`); `RFC §10.3`; FND-07 §3.1 |

...
> `github.com`. Ficam substituídos os itens deste ADR sobre host de
...
`dmpf` separa versionamento (`nx release --skip-publish` + push de
...
- **`publish-libs.yml`**: dispara em tags `**@*` e `workflow_dispatch`; chama
  ...
  com Node 24 / pnpm 11, `nx_exclude: @mateusmacedo/dmpf-source`,
  `scope: @mateusmacedo`, `build_projects_filter: tag:type:lib`.
- **Runner `gitea-runner`**: adoção deliberada da org (parity de label com
  monorepo de referência). Não implica API Gitea nem troca de host GitHub.
...
- **Sem `publishConfig` nas libs**: parity com o monorepo de referência; registry vem do auth
  do reusable. O escopo dos pacotes (`@mateusmacedo`) precisa bater com o
...
| Item do monorepo de referência | Motivo de não portar |
|------------------|----------------------|
...
- Consumidores do template precisam do reusable `publish-libs.yaml` acessível
  em `mateusmacedo/actions-templates@main` e da role/SSM de auth Verdaccio
  configurada na org.
- Docker release permanece no mesmo workflow de versionamento (vantagem do
  template ausente no monorepo de referência).
- CI estrutural (`ci.yml`) e create-release não mudam nesta entrega.

## Referências

- Spec: `docs/specs/SPEC-W4RWD02M-evoluir-workflows-com-monorepo-referencia.md`
- ADR-003 (hardening / follow-up de registry)
- Referência: `dmpf/.github/workflows/{release,publish-libs}.yml`

...
| ADR-044 | Separar a referência em um BFF REST público e dois contextos com gRPC interno | Substitui o `dmpf-reference` único por três composition roots e seis processos: `dmpf-reference-bff-go` é a única borda REST, em `bounded_context` próprio e sem importar domínio nem aplicação; `dmpf-reference-orders-go` e `dmpf-reference-reservations-go` servem gRPC e drenam cada um a outbox do próprio banco. Os `.proto` de serviço geram só mensagens e descriptor, porque o bloco `contract` não admite `io.network`; o binding vive no bloco `app`. Prazo e contexto de mensagem nascem na borda, retry só nos `Find*`, `Canceled` como segundo estado terminal de `reservations` e e2e caixa-preta com seis processos. A saúde fica fora da cadeia de interceptors do contexto. No kernel, `resilience.Breaker` ganha `CountsAsFailure` e o `dmpf-provider-grpc` só conta indisponibilidade, então `NOT_FOUND` não abre o breaker. |

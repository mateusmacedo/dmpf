...
| ADR-048 | Layout canônico de bounded context, kernel de composição e gate de estrutura | Fixa o layout canônico do bounded context — raiz sem código Go, borda do transporte em subpacote de `app/` (`app/rpc/`, `app/http/`), binário único em `cmd/main.go` —, promove onze capacidades duplicadas entre `orders` e `reservations` ao kernel (`postgres`, `grpc`, `admission`, `relay`, `transport/channel`, `kafka`, `observability`), reverte a exclusão de composition root em `bookings` — que ganha `app/config.go`, `app/wiring.go`, `cmd/main.go` e os targets `serve-api`/`serve-relay` —, torna `appkit` e `distkit` obrigatórios em todo contexto e entrega `tools/dmpf-context-check.sh` como gate de estrutura. |
...
| ADR-052 | Verificar a identidade do workload no gRPC por mTLS e no Kafka por SASL | A permissão do sujeito é conferida na borda, por rota; cada salto interno autentica o workload — mTLS com allowlist no gRPC, antes de ler `x-tenant-id`, e SASL ou certificado de cliente no Kafka, com a ligação principal × `source` pela ACL do broker. `TransportVerified` só vale com TLS e cliente autenticado. |
| ADR-053 | Dar banco próprio a cada contexto, com nomes canônicos, e uma forma única aos três contextos | Database e role com o nome da app, tabelas sem prefixo (`outbox`, agregado no plural), migrate por capacidade e persistência híbrida (colunas só para o que é consultado, resto em `snapshot` `jsonb`). O `bookings` passa a gRPC, com o REST no `bff`; instante em nanossegundos, fio preservado, config sem `DMPF_`, `bearerAuth` nos OpenAPI e imports agrupados pelo `gci`. |

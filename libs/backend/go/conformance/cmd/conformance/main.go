// Command conformance decide a regra de dependência do DMPF sobre o grafo
...
func main() {
	...
	perfis := flag.String("profiles", "", "caminho do build-profiles.json (default: <root>/libs/backend/go/conformance/build-profiles.json)")
...
func run(o opcoes, saida, erros io.Writer) int {
	...
	if err != nil {
		_, _ = fmt.Fprintf(erros, "conformance: %v\n", err)
		return exitFalha
	}
	...
	if err := w.Flush(); err != nil {
		_, _ = fmt.Fprintf(erros, "conformance: escrever relatório: %v\n", err)
		return exitFalha
	}
...
func preparar(o opcoes) (string, string, error) {
	...
	perfis := o.perfis
	if perfis == "" {
		perfis = filepath.Join(abs, "libs", "backend", "go", "conformance", "build-profiles.json")
	}

...
func FirstReception(p Pending) Reception {
	if p == nil {
		panic("dmpfports: FirstReception requires a non-nil Pending")
	}
...
func (r Reception) Match(
	...
	if first == nil || processed == nil || rejected == nil || collision == nil {
		panic("dmpfports: Reception.Match requires all four branches")
	}
	switch r.kind {
	case receptionFirst:
	...
	case receptionCollision:
		return collision()
	default:
		panic("dmpfports: Reception zero value has no classification")
	}
}
...
var ErrRegisterTimeout = errors.New("dmpfports: inbox register wait exceeded")
...
var ErrPendingNotCompleted = errors.New("dmpfports: first reception left without completion")

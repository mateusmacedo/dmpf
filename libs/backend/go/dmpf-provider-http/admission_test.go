...
func TestAdmissionRefusesWith429BeforeReadingTheBody(t *testing.T) {
	...
	middleware := dmpfhttp.Admission(ctrl, nil, tenantHeader, instruments)(handler)
...
func TestAdmissionResolvesTenantsAgainstTheAllowlist(t *testing.T) {
	...
	middleware := dmpfhttp.Admission(ctrl, nil, tenantHeader, nil)(ok)
...
func TestAdmissionAnswers404ForARouteWithoutLimit(t *testing.T) {
	...
	middleware := dmpfhttp.Admission(ctrl, nil, nil, nil)(handler)
...
func TestAdmissionReleasesTheSlotAfterTheHandler(t *testing.T) {
	...
	ok := http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) { w.WriteHeader(http.StatusNoContent) })
	middleware := dmpfhttp.Admission(ctrl, nil, nil, nil)(ok)
...
func TestAdmissionUsesTheInjectedRouteFunc(t *testing.T) {
	...
	middleware := dmpfhttp.Admission(ctrl, byPattern, nil, nil)(ok)
...
func TestAnUndeclaredRouteIsCountedUnderOneLabelNotItsPath(t *testing.T) {
	...
	middleware := dmpfhttp.Admission(ctrl, nil, nil, instruments)(http.HandlerFunc(func(http.ResponseWriter, *http.Request) {}))
	...
	for _, scope := range collected.ScopeMetrics {
		for _, m := range scope.Metrics {
			...
			route, _ := sum.DataPoints[0].Attributes.Value(metrics.KeyRoute)
			if route.AsString() != dmpfhttp.UndeclaredRouteLabel {
				t.Fatalf("route = %q, want %q", route.AsString(), dmpfhttp.UndeclaredRouteLabel)
			}
			return
		}
	}

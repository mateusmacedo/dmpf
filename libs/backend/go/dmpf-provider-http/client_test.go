...
func clientConfig(routes ...dmpfhttp.Route) dmpfhttp.Config {
	...
	byName := map[string]dmpfhttp.Route{}
	for _, r := range routes {
		byName[r.Name] = r
	}
	return dmpfhttp.Config{
		Routes:  byName,
		Sheet:   sheet,
		Service: "checkout",
		Clock:   clock.System(),
...
func newClient(t *testing.T, cfg dmpfhttp.Config) *dmpfhttp.Client {
	t.Helper()
	client, err := dmpfhttp.NewClient(cfg)
	if err != nil {
...
func TestNewClientRefusesAnInvalidConfig(t *testing.T) {
	t.Run("route without contract", func(t *testing.T) {
		...
		r.ContractRef = ""
		if _, err := dmpfhttp.NewClient(clientConfig(r)); !errors.Is(err, dmpfhttp.ErrContractRequired) {
			t.Fatalf("NewClient() = %v, want ErrContractRequired (RST-04)", err)
		}
	})
	t.Run("no routes", func(t *testing.T) {
		if _, err := dmpfhttp.NewClient(clientConfig()); !errors.Is(err, dmpfhttp.ErrIncompleteConfig) {
	...
	t.Run("no clock", func(t *testing.T) {
		...
		cfg.Clock = nil
		if _, err := dmpfhttp.NewClient(cfg); !errors.Is(err, dmpfhttp.ErrIncompleteConfig) {
			t.Fatalf("NewClient() = %v, want ErrIncompleteConfig", err)
		}
	})
}
...
func TestDoRefusesBeforeAnyRequest(t *testing.T) {
	...
	t.Run("undeclared route", func(t *testing.T) {
		...
		if !errors.Is(err, dmpfhttp.ErrRouteNotDeclared) {
	...
	t.Run("method mismatch", func(t *testing.T) {
		...
		if !errors.Is(err, dmpfhttp.ErrMethodMismatch) {
	...
	t.Run("non-replayable body on a retryable route", func(t *testing.T) {
		...
		if !errors.Is(err, dmpfhttp.ErrBodyNotReplayable) {
...
func TestClassifier(t *testing.T) {
	cases := map[string]struct {
		...
		"transient status": {&dmpfhttp.RetryableStatusError{Status: 503}, retry.Retryable},
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			if got := dmpfhttp.Classifier(tc.err); got != tc.want {

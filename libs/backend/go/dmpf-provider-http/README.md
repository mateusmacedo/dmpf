...
| --- | --- | --- |
| `dmpf-kernel/provider-http` | `provider` | raiz do módulo |
...
- **`admission.go`** — `Admission(ctrl, route, tenant, instruments)`:
  ...
  `dmpf-transport/admission`. Recusa **429** com `Retry-After` antes do
...
```bash
pnpm nx run dmpf-provider-http-go:test-race
```

## Validação

```bash
pnpm nx run-many -t fmt-check,vet,lint,build,test-race,govulncheck -p dmpf-provider-http-go
...
- `libs/backend/go/dmpf-transport/README.md` — `deadline`, `admission`, `observe`.

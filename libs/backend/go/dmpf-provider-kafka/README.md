...
| --- | --- | --- |
| `dmpf-kernel/provider-kafka` | `provider` | raiz do módulo |
...
- **`publisher.go` / `record.go`** — `Publisher.Publish(ctx, destino, bytes)`
  ...
  (`dmpf-transport/compose`), com acks de todas as réplicas em sincronia e
  escrita idempotente.
...
- **`dlq.go`** — `DLQ` realiza `dmpfports.Containment`: publica o envelope
...
pnpm nx run dmpf-provider-kafka-go:test-race
```
...
```bash
pnpm nx run-many -t fmt-check,vet,lint,build,test-race,govulncheck -p dmpf-provider-kafka-go
...
- `libs/backend/go/dmpf-transport/README.md` — `channel`, `attempt`, `observe`.

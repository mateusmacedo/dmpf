...
type scriptedSink struct {
	mu      sync.Mutex
	calls   []handled
	script  map[int64]func(ctx context.Context, attempt int, ack dmpfports.Acknowledger) error
	byValue map[string]int64
}

func newSink() *scriptedSink {
	return &scriptedSink{script: map[int64]func(context.Context, int, dmpfports.Acknowledger) error{}, byValue: map[string]int64{}}
}

func (s *scriptedSink) on(offset int64, fn func(ctx context.Context, attempt int, ack dmpfports.Acknowledger) error) {
	s.script[offset] = fn
...
func alwaysAck(ctx context.Context, _ int, ack dmpfports.Acknowledger) error { return ack.Ack(ctx) }

func alwaysRelease(ctx context.Context, _ int, ack dmpfports.Acknowledger) error {
	return ack.Release(ctx)
}

func newConsumer(sink dmpfkafka.Sink) *dmpfkafka.Consumer {
	cfg := publishConfig()
	cfg.Clock = clock.System()
	cfg.Rand = func() float64 { return 0 }
	return &dmpfkafka.Consumer{
		Config:             cfg,
...
func consume(t *testing.T, c *dmpfkafka.Consumer, fake *dmpfkafka.FakeClient, until func() bool) {
	t.Helper()
...
func commits(fake *dmpfkafka.FakeClient) []int64 {
...
func TestConsumerValidate(t *testing.T) {
	cases := map[string]func(*dmpfkafka.Consumer){
		"no sink":                      func(c *dmpfkafka.Consumer) { c.Sink = nil },
		"no attempt limit":             func(c *dmpfkafka.Consumer) { c.Channel.Retry.MaxAttempts = 0 },
		"no queue":                     func(c *dmpfkafka.Consumer) { c.QueuePerPartition = 0 },
		"deadline not below rebalance": func(c *dmpfkafka.Consumer) { c.ProcessingDeadline = c.RebalanceTimeout },
		"no backoff":                   func(c *dmpfkafka.Consumer) { c.Backoff = retry.Backoff{} },
		"not kafka":                    func(c *dmpfkafka.Consumer) { c.Channel = sqsChannel() },
	}
...
func TestARecordWithoutGestureStallsThePartitionAndFixesTheCommitCeiling(t *testing.T) {
	sink := newSink()
	sink.on(0, alwaysAck)
	sink.on(1, func(context.Context, int, dmpfports.Acknowledger) error { return nil }) // no gesture
	sink.on(2, alwaysAck)
	fake := dmpfkafka.NewFakeClient()
	fake.Feed(topic, 0, recordsAt(0, 1, 2)...)
	c := newConsumer(sink)
...
func TestReleaseRedeliversWithTheNextAttempt(t *testing.T) {
	sink := newSink()
	sink.on(0, func(ctx context.Context, attempt int, ack dmpfports.Acknowledger) error {
		if attempt == 1 {
			return ack.Release(ctx)
		}
		return ack.Ack(ctx)
...
func TestAFaultBetweenQuarantineAndAckDoesNotAdvanceTheOffsetNorRepeatTheQuarantine(t *testing.T) {
	dlqFake := dmpfkafka.NewFakeClient()
	dlq, err := dmpfkafka.NewDLQWith(publishConfig(), ordersChannel(), dlqFake)
	if err != nil {
		t.Fatal(err)
	}
	raw, _ := validRaw(t, "k1")
	...
	sink := newSink()
	sink.on(0, func(ctx context.Context, _ int, _ dmpfports.Acknowledger) error {
		if err := dlq.Quarantine(ctx, dmpfports.Contained{Consumer: "billing", Reason: dmpfports.ReasonTerminalFailure, Envelope: raw}); err != nil {
			return err
		}
	...
	if headers[dmpfkafka.HeaderAttempt] != "1" {
		t.Fatalf("dmpf-attempt = %q, want the attempt the consumer recorded in the context (TRP-52)", headers[dmpfkafka.HeaderAttempt])
	}
}
...
func TestRevocationCancelsTheAttemptAndKeepsTheContiguousCommit(t *testing.T) {
	...
	sink := newSink()
	sink.on(0, alwaysAck)
	sink.on(1, func(ctx context.Context, _ int, _ dmpfports.Acknowledger) error {
		close(blocked)
		<-ctx.Done()
		observed <- ctx.Err()
		return ctx.Err()
...
func TestRevocationLiftsTheFetchPauseOfAStalledPartition(t *testing.T) {
	sink := newSink()
	sink.on(0, func(context.Context, int, dmpfports.Acknowledger) error { return nil }) // no gesture
	fake := dmpfkafka.NewFakeClient()
	fake.Feed(topic, 0, recordsAt(0, 1)...)
	c := newConsumer(sink)
...
func TestRevocationDoesNotDrainTheQueueThroughTheSink(t *testing.T) {
	...
	slow := func(ctx context.Context, _ int, ack dmpfports.Acknowledger) error {
		started <- struct{}{}
		select {
		case <-ctx.Done():
			return ctx.Err()
...
func TestASinkPanicIsAFailedAttemptNotACrash(t *testing.T) {
	sink := newSink()
	sink.on(0, func(context.Context, int, dmpfports.Acknowledger) error { panic("bad envelope") })
	sink.on(1, alwaysAck)
	fake := dmpfkafka.NewFakeClient()
	fake.Feed(topic, 0, recordsAt(0, 1)...)
	c := newConsumer(sink)

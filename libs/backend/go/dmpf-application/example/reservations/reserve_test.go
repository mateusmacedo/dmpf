...
func TestReserveCreatesTheReservationAndAuthorsTheOutboxEntry(t *testing.T) {
	...
	out, err := h.service.Reserve(context.Background(), reservationsapp.Reserve{Order: syncOrder, Items: 2})
	...
	if got, want := out.Response(), (reservations.ReservedResponse{Order: syncOrder, Items: 2}); got != want {
	...
	if version != 1 || snapshot.Status != reservations.Confirmed || snapshot.Items != 2 {
	...
	want := dmpfports.OutboxEntry{
		...
		Intent:           dmpfports.PublishIntent{Destination: reservationsapp.Destination, PartitionKey: string(syncOrder)},
		AggregateType:    reservationsapp.AggregateType,
		AggregateID:      string(syncOrder),
		AggregateVersion: 1,
		Event:            reservations.ReservationConfirmed{Order: syncOrder, Items: 2, At: reservations.Instant(syncOccurred.Unix())},
...
func TestReserveCopiesTheMessageContextIntoTheOutboxEntry(t *testing.T) {
	...
	ctx := dmpfports.WithMessageContext(context.Background(), dmpfports.MessageContext{
		CorrelationID: "corr-1", CausationID: "req-ctx-1", Traceparent: traceparent,
	})

	if _, err := h.service.Reserve(ctx, reservationsapp.Reserve{Order: syncOrder, Items: 1}); err != nil {
		t.Fatalf("Reserve() error = %v, want nil", err)
	}
	...
	want := dmpfports.MessageContext{CorrelationID: "corr-1", CausationID: "req-ctx-1", Traceparent: traceparent}
...
func TestReserveOnACanceledReservationRejectsWithoutWriting(t *testing.T) {
	...
	out, err := h.service.Reserve(context.Background(), reservationsapp.Reserve{Order: syncOrder, Items: 1})

	if err != nil {
		t.Fatalf("Reserve() error = %v, want nil — a refusal is not a technical failure (DEC-04)", err)
	}
	rej, refused := out.Rejection()
	if !refused || rej.Code() != reservations.CodeReservationCanceled {
		t.Fatalf("Rejection() = %v, %v; want %q", rej, refused, reservations.CodeReservationCanceled)
	}
	if _, version, _ := h.store.ReservationsReader().Load(context.Background(), syncOrder); version != 1 {
...
func TestReserveUnderAVersionConflictStopsBeforeTheOutbox(t *testing.T) {
	h := newSyncHarness(t, withSyncSaveError(dmpfports.ErrVersionConflict))

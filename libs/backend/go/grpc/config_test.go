...
func policy(idempotent bool) grpc.MethodPolicy {
	return grpc.MethodPolicy{
...
func validConfig() grpc.Config {
	return grpc.Config{
		TLS:     &tls.Config{MinVersion: tls.VersionTLS13},
		Sheet:   resilience.Defaults("orders"),
		Methods: map[string]grpc.MethodPolicy{checkMethod: policy(true)},
...
func TestConfigValidate(t *testing.T) {
	...
	t.Run("without TLS and without the opt-out", func(t *testing.T) {
		cfg := validConfig()
		cfg.TLS = nil
		if err := cfg.Validate(); !errors.Is(err, grpc.ErrTLSRequired) {
	...
	t.Run("TLS that skips verification or admits less than 1.2", func(t *testing.T) {
		for name, weak := range map[string]*tls.Config{"skip verify": {InsecureSkipVerify: true}, "tls 1.0": {MinVersion: tls.VersionTLS10}} {
			cfg := validConfig()
			cfg.TLS = weak
			if err := cfg.Validate(); !errors.Is(err, grpc.ErrTLSTooWeak) {
				t.Fatalf("%s: Validate() = %v, want ErrTLSTooWeak (GRP-15)", name, err)
			}
			if err := (grpc.ServerConfig{TLS: weak}).Validate(); !errors.Is(err, grpc.ErrTLSTooWeak) {
	...
	t.Run("without clock", func(t *testing.T) {
		cfg := validConfig()
		cfg.Clock = nil
		if err := cfg.Validate(); !errors.Is(err, grpc.ErrIncompleteConfig) {

...
func Admission(ctrl *admission.Controller, tenant TenantFunc, instruments *metrics.Instruments) grpc.UnaryServerInterceptor {
	return func(ctx context.Context, req any, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (any, error) {
		...
		if reason == admission.UndeclaredRoute {
			return nil, status.Errorf(codes.Unimplemented, "grpc: %s declares no admission limit (RES-16)", info.FullMethod)
		}
		return nil, status.Errorf(codes.ResourceExhausted, "grpc: admission refused: %s (RES-17)", reason)
	}
}

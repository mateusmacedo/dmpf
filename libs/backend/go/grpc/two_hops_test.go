...
func routeConfig(limit, slack time.Duration) provider.Config {
...
func chain(t *testing.T, cfg provider.Config, blockC <-chan struct{}) (healthpb.HealthClient, chan deadlineSeen) {
	t.Helper()
	seen := make(chan deadlineSeen, 8)

	hopC := &hop{name: "C", deadlines: seen, block: blockC}
	dialC := serve(t, hopC)
	connC := connect(t, dialC, grpc.WithChainUnaryInterceptor(provider.DeadlineUnaryInterceptor(cfg)))

	hopB := &hop{name: "B", deadlines: seen, next: healthpb.NewHealthClient(connC)}
	dialB := serve(t, hopB)
	connB := connect(t, dialB, grpc.WithChainUnaryInterceptor(provider.DeadlineUnaryInterceptor(cfg)))

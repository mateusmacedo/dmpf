...
func TestServiceConfigDeclaresRoundRobinAndHealthCheck(t *testing.T) {
	...
	if err := json.Unmarshal([]byte(dmpfgrpc.ServiceConfig("orders.v1.Orders")), &decoded); err != nil {
...
func TestDialRefusesAnInvalidConfig(t *testing.T) {
	cfg := validConfig()
	cfg.TLS = nil
	if _, err := dmpfgrpc.Dial("passthrough:///bufnet", cfg); !errors.Is(err, dmpfgrpc.ErrTLSRequired) {
...
func TestDialAppliesDeadlineAndCompositionToTheCall(t *testing.T) {
	...
	conn, err := dmpfgrpc.Dial("passthrough:///bufnet", cfg, grpc.WithContextDialer(dialer))
	if err != nil {
...
func TestNewServerRefusesToRunWithoutTLSOrOptOut(t *testing.T) {
	if _, _, err := dmpfgrpc.NewServer(dmpfgrpc.ServerConfig{}); !errors.Is(err, dmpfgrpc.ErrTLSRequired) {

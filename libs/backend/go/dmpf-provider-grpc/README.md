...
| --- | --- | --- |
| `dmpf-kernel/provider-grpc` | `provider` | raiz do módulo |
...
- **`observe.go`** — liga o módulo a `dmpf-transport/observe`, que realiza os
...
- **`admission.go`** — `Admission(ctrl, tenant, instruments)`: interceptor do
  servidor sobre `dmpf-transport/admission`. Recusa `RESOURCE_EXHAUSTED` antes
...
```bash
pnpm nx run dmpf-provider-grpc-go:test-race
```

## Validação

```bash
pnpm nx run-many -t fmt-check,vet,lint,build,test-race,govulncheck -p dmpf-provider-grpc-go
...
- `libs/backend/go/dmpf-transport/README.md` — `deadline`, `admission` e `observe`.
- `libs/backend/go/dmpf-observability/README.md` — a composição do KRN-09.

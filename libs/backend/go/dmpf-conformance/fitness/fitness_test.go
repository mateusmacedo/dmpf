...
// The edge dmpf-testkit (dmpf-kernel, app) -> dmpf-conformance/fitness
// (dmpf-conformance, app): C1 holds (app -> app), and C2 holds only because the
...
func TestFitnessUnitIsReachableAcrossBoundedContexts(t *testing.T) {
	const (
		modKit  = "exemplo/dmpf-testkit"
		modConf = "exemplo/dmpf-conformance"
		pkgKit  = "exemplo/dmpf-testkit/fitness"
		pkgFit  = "exemplo/dmpf-conformance/fitness"
	)
	mods := []fitness.Module{
	...
	kit := fitness.Unit{ID: "dmpf-kernel/testkit-fitness", Block: fitness.BlockApp, BoundedContext: "dmpf-kernel", Include: []string{pkgKit}, Module: modKit}
	...
	t.Run("positivo: unidade fitness com superfície pública", func(t *testing.T) {
		fit := fitness.Unit{ID: "dmpf-conformance/fitness", Block: fitness.BlockApp, BoundedContext: "dmpf-conformance", PublicIntegrationSurface: true, Include: []string{pkgFit}, Module: modConf}
	...
	t.Run("negativo: sem superfície pública reprova por D002", func(t *testing.T) {
		fit := fitness.Unit{ID: "dmpf-conformance/fitness", Block: fitness.BlockApp, BoundedContext: "dmpf-conformance", Include: []string{pkgFit}, Module: modConf}

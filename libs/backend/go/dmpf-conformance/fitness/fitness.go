...
// Workspace is the gate's own composition (cmd/dmpf-conformance) minus the
...
func Workspace(root, profilesPath string) (Input, error) {
	...
	if profilesPath == "" {
		profilesPath = filepath.Join(abs, "libs", "backend", "go", "dmpf-conformance", "build-profiles.json")
	}

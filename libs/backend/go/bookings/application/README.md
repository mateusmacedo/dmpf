# bookings-application-go

Bloco `application` do bounded context `resource-scheduling`, gerado pelo generator
...
Projeto Nx `bookings-application-go`, tags `type:lib`, `scope:backend`, `stack:go` e
`layer:services`. Import path do módulo:
...
| --- | --- | --- | --- |
| `resource-scheduling/application` | `application` | `resource-scheduling` | `bookingsapplication` (raiz) |

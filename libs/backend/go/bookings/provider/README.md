# bookings-provider-postgres-go

Bloco `provider` do bounded context `resource-scheduling`, gerado pelo generator
...
Projeto Nx `bookings-provider-postgres-go`, tags `type:lib`, `scope:backend`, `stack:go` e
...
| --- | --- | --- | --- |
| `resource-scheduling/provider-postgres` | `provider` | `resource-scheduling` | `bookingspostgres` (raiz) |

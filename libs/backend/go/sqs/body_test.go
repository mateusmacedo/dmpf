...
func TestDecodeBodyRefusesTheSNSWrapper(t *testing.T) {
	...
	if _, err := sqs.DecodeBody(wrapper); !errors.Is(err, sqs.ErrSNSEnvelopeNotRaw) {
		t.Fatalf("DecodeBody(wrapper) = %v, want ErrSNSEnvelopeNotRaw (SQS-02)", err)
	}
	if _, err := sqs.DecodeBody(`{"Type":"Other"}`); !errors.Is(err, sqs.ErrInvalidBody) {
		t.Fatalf("DecodeBody(other json) = %v, want ErrInvalidBody", err)
	}
	if _, err := sqs.DecodeBody("not base64!"); !errors.Is(err, sqs.ErrInvalidBody) {

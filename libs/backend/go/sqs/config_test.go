...
func TestConfigValidate(t *testing.T) {
	cases := map[string]struct {
		mutate func(*sqs.Config)
		want   error
	}{
		"complete":                    {func(*sqs.Config) {}, nil},
		"no region":                   {func(c *sqs.Config) { c.AWS.Region = "" }, sqs.ErrIncompleteConfig},
		"no clock":                    {func(c *sqs.Config) { c.Clock = nil }, sqs.ErrIncompleteConfig},
		"no catalogue":                {func(c *sqs.Config) { c.Catalog = nil }, sqs.ErrIncompleteConfig},
		"plaintext endpoint":          {func(c *sqs.Config) { c.Endpoint = "http://localhost:4566" }, sqs.ErrTLSRequired},
		"plaintext endpoint, opt-out": {func(c *sqs.Config) { c.Endpoint, c.InsecureForDevelopmentOnly = "http://localhost:4566", true }, nil},
		"https endpoint":              {func(c *sqs.Config) { c.Endpoint = "https://sqs.us-east-1.amazonaws.com" }, nil},
		"blank sheet":                 {func(c *sqs.Config) { c.Sheet.Breaker = resilience.Field[resilience.BreakerPolicy]{} }, resilience.ErrBlankField},
	}
...
func TestConfigChannel(t *testing.T) {
	...
	if _, err := cfg.Channel("payments"); !errors.Is(err, sqs.ErrUnknownChannel) {
		t.Fatalf("Channel(unknown) = %v, want ErrUnknownChannel (ASY-01)", err)
	}
	if _, err := cfg.Channel("orders"); !errors.Is(err, sqs.ErrNotSQSChannel) {
	...
	t.Run("fifo without group ordering is refused", func(t *testing.T) {
		...
		if _, err := cfg.Channel(ch.Name); !errors.Is(err, sqs.ErrOrderingMismatch) {

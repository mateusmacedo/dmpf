...
func TestNewSNSPublisherDiscoversTheSubscriptionsAndRequiresRawDelivery(t *testing.T) {
	...
	_, err := sqs.NewSNSPublisher(context.Background(), snsOnlyConfig(), api)
	if !errors.Is(err, sqs.ErrRawDeliveryRequired) {
...
func TestNewSNSPublisherIgnoresNonSQSSubscriptionsButRequiresOne(t *testing.T) {
	...
	if _, err := sqs.NewSNSPublisher(context.Background(), snsOnlyConfig(), api); !errors.Is(err, sqs.ErrNoSubscriptions) {
		t.Fatalf("NewSNSPublisher() = %v, want ErrNoSubscriptions", err)
	}
	api.Subscribe(topicARN, subARN, "sqs", map[string]string{"RawMessageDelivery": "true"})
	if _, err := sqs.NewSNSPublisher(context.Background(), snsOnlyConfig(), api); err != nil {
...
func TestNewSNSPublisherRefusesAFilterOnAFIFOTopic(t *testing.T) {
	...
	if _, err := sqs.NewSNSPublisher(context.Background(), cfg, api); !errors.Is(err, sqs.ErrFilterOnFifoTopic) {
		t.Fatalf("NewSNSPublisher() = %v, want ErrFilterOnFifoTopic (SQS-03c)", err)
	}

	standard := sqs.NewFakeSNS()
	standard.Subscribe(topicARN, subARN, "sqs", map[string]string{"RawMessageDelivery": "true", "FilterPolicy": `{"kind":["a"]}`})
	if _, err := sqs.NewSNSPublisher(context.Background(), snsOnlyConfig(), standard); err != nil {
...
func TestSNSPublishSendsTheSameBodyAndAttributes(t *testing.T) {
	api := sqs.NewFakeSNS()
	api.Subscribe(topicARN, subARN, "sqs", map[string]string{"RawMessageDelivery": "true"})
	pub, err := sqs.NewSNSPublisher(context.Background(), validConfig(), api)
	if err != nil {
	...
	if *in.TopicArn != topicARN || *in.Message != sqs.EncodeBody(raw) {
	...
	if _, ok := in.MessageAttributes[sqs.AttrPublishedAt]; !ok {

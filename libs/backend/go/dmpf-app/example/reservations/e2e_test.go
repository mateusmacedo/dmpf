...
const (
	e2eOccurred = dmpfports.Instant(1_757_000_000_000_000_000)
...
func (s *sequenceIDs) NewMessageID() dmpfports.MessageID {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.next++
	return dmpfports.MessageID(fmt.Sprintf("m-%06d", s.next))
}
...
type recordingAck struct {
	...
	releases      int
	inboxAtAck    int64
	messageIDSeen dmpfports.MessageID
}
...
func quarantinedEnvelope(t *testing.T, pool *pgxpool.Pool, messageID string) ([]byte, string) {
	...
	err := pool.QueryRow(context.Background(),
		...
		reservationsconsumer.ConsumerName, messageID).Scan(&raw, &reason)
	if err != nil {
...
func consume(t *testing.T, pool *pgxpool.Pool, consumer dmpfapp.Consumer, messageID string, raw []byte, attempt int) (dmpfapp.Outcome, error, *recordingAck) {
	t.Helper()
	ack := &recordingAck{pool: pool, messageIDSeen: dmpfports.MessageID(messageID)}
	outcome, err := consumer.Consume(context.Background(), dmpfapp.Delivery{Raw: raw, Attempt: attempt}, ack)
...
func (f failingOutbox) Enqueue(context.Context, dmpfports.OutboxEntry) error { return f.err }

func consumerWith(pool *pgxpool.Pool, decorate func(reservationsapp.Resources) reservationsapp.Resources) dmpfapp.Consumer {
	bind := func(tx *dmpfpostgres.Tx) reservationsapp.Resources {
		return decorate(reservationsconsumer.Bind(e2eWait)(tx))
	}
	...
	return dmpfapp.Consumer{
		Name:        reservationsconsumer.ConsumerName,
		MaxAttempts: e2eAttempts,
		Handle:      reservationsconsumer.Handler(service),
...
func TestFirstReceptionAppliesConfirmsAndDerivesTheOutbox(t *testing.T) {
	...
	if outcome.Disposition != dmpfapplication.R1D1 || outcome.Contained {
...
func TestBusinessRejectionCommitsRejectedAndConfirms(t *testing.T) {
	...
	if outcome.Disposition != dmpfapplication.R1D2 || outcome.Contained {
		t.Fatalf("outcome = %+v", outcome)
	}
	...
	if status != "rejected" || lastError == nil || *lastError != string(reservations.CodeNothingToReserve) {
		t.Fatalf("inbox = (%s, %v), want (rejected, %s)", status, lastError, reservations.CodeNothingToReserve)
	}
...
func TestRedeliveriesShortCircuit(t *testing.T) {
	...
	t.Run("R2 reapplies nothing", func(t *testing.T) {
		...
		if err != nil || outcome.Disposition != dmpfapplication.R2 || outcome.Contained {
	...
	t.Run("R3 re-emits no rejection event", func(t *testing.T) {
		...
		if err != nil || outcome.Disposition != dmpfapplication.R3 || outcome.Contained {
			t.Fatalf("outcome = %+v, err = %v", outcome, err)
		}
		if ack.acks != 1 || counts(t, pool) != before {
	...
	t.Run("R4 contains the collision byte for byte", func(t *testing.T) {
		...
		if reason != string(dmpfports.ReasonCollision) || !bytes.Equal(raw, collision) {
...
func TestTransientFailureRollsBackAndReleases(t *testing.T) {
	pool := openPool(t)
	consumer := consumerWith(pool, func(r reservationsapp.Resources) reservationsapp.Resources {
...
func TestTerminalFailureIsContained(t *testing.T) {
	pool := openPool(t)
	consumer := consumerWith(pool, func(r reservationsapp.Resources) reservationsapp.Resources {
		r.Reservations = failingReservations{r.Reservations, dmpfapplication.NewFailure(dmpfapplication.Forbidden, false, errBoom)}
		return r
	})
...
func TestPayloadOfAnotherContractIsTerminal(t *testing.T) {
	...
	if !errors.Is(err, envelope.ErrSchemaMismatch) || outcome.Disposition != dmpfapplication.R1D4 || outcome.Reason != dmpfports.ReasonTerminalFailure {
		t.Fatalf("outcome = %+v, err = %v", outcome, err)
	}
	if got := counts(t, pool); got != (tableCounts{quarantine: 1}) {
...
func TestRedeliveryWithANewMessageIDDoesNotDuplicateTheEffect(t *testing.T) {
	...
	if status, lastError := inboxRow(t, pool, "evt-9"); status != "rejected" || lastError == nil || *lastError != string(reservations.CodeAlreadyReserved) {
...
func TestSignalsExposeTheConsumerSide(t *testing.T) {
	...
	signals, err := dmpfpostgres.InboxSignals(context.Background(), pool, reservationsconsumer.ConsumerName)
	if err != nil {

...
func receipt(consumer string, id ports.MessageID, hash string) ports.Receipt {
	return ports.Receipt{Consumer: consumer, MessageID: id, MessageType: "example", PayloadHash: hash, ReceivedAt: 100}
}

// match drives r.Match, completing the first branch as completeAs when non-nil,
// and reports which of the four branches ran.
func match(r ports.Reception, completeAs *ports.Status) (branch string, err error) {
	err = r.Match(
		func(p ports.Pending) error {
			...
			return p.Complete(context.Background(), ports.Completion{Status: *completeAs, At: 200})
		},
...
func statusPtr(s ports.Status) *ports.Status { return &s }
...
func Inbox(newSubject func() InboxSubject) Verdict {
	...
	registerAndCommit := func(clause string, s InboxSubject, consumer string, id ports.MessageID, hash string, status ports.Status) bool {
		err := s.Within(context.Background(), consumer, func(ctx context.Context, inbox ports.Inbox) error {
	...
	redeliver := func(s InboxSubject, consumer string, id ports.MessageID, hash string) (string, error) {
		var branch string
		err := s.Within(context.Background(), consumer, func(ctx context.Context, inbox ports.Inbox) error {
			reception, err := inbox.Register(ctx, receipt(consumer, id, hash))
			if err != nil {
...

	{
		...
		s := newSubject()
		if registerAndCommit(clause, s, "orders", "m-1", "h1", ports.StatusProcessed) {
			if status, ok := s.ReadStatus("orders", "m-1"); !ok || status != ports.StatusProcessed {
	...
	for _, c := range []struct {
		...
		{"redelivery with the same hash after a processed commit is R2", "INB-06", ports.StatusProcessed, "processed"},
		{"redelivery with the same hash after a rejected commit is R3", "INB-12", ports.StatusRejected, "rejected"},
	} {
		s := newSubject()
...

	{
		...
		s := newSubject()
		err := s.Within(context.Background(), "orders", func(ctx context.Context, inbox ports.Inbox) error {
			reception, err := inbox.Register(ctx, receipt("orders", "m-1", "h1"))
			if err != nil {
				return err
			}
			if _, err := match(reception, statusPtr(ports.StatusProcessed)); err != nil {
				return err
			}
		...
		if !errors.Is(err, errInboxRollback) {
			...
			err := s.Within(context.Background(), "orders", func(ctx context.Context, inbox ports.Inbox) error {
				...
				branch, err = match(reception, statusPtr(ports.StatusProcessed))
				return err
			})
			if err != nil {
...

	{
		...
		s := newSubject()
		if registerAndCommit(clause, s, "orders", "m-1", "h1", ports.StatusProcessed) {
			if !registerAndCommit(clause, s, "billing", "m-1", "h1", ports.StatusProcessed) {
...

	{
		...
		if !s.Concurrent {
			...
			} else if status, ok := s.ReadStatus("orders", "m-race"); !ok || status != ports.StatusProcessed {
...
func race(s InboxSubject, consumer string, id ports.MessageID, hash string) (winners int, err error) {
	const attempts = 2
	...
	for range attempts {
		...
		go func() {
			...
			attempt := func() (branch string, err error) {
				err = s.Within(ctx, consumer, func(ctx context.Context, inbox ports.Inbox) error {
					reception, err := inbox.Register(ctx, receipt(consumer, id, hash))
					if !signalled {
						signalled = true
						registered.Done()

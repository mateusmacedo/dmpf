...
// OutboxStore is the drain side of the outbox as the relay of app declares
...
type OutboxSubject[C any] struct {
	...
	Purge   func(before ports.Instant) (removed int64, err error)
}
...
func Outbox[C any](newSubject func() OutboxSubject[C]) Verdict {
...

	{
		...
		if err := s.Enqueue(1); err != nil {
			...
			later := s.Clock.Now() + ports.Instant(30*time.Second)

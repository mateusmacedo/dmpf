...
func (g *Sequence) NewMessageID() ports.MessageID {
	return ports.MessageID(g.next())
}
...
func (g *Seeded) NewMessageID() ports.MessageID {
	g.mu.Lock()
	defer g.mu.Unlock()
	return ports.MessageID(fmt.Sprintf("%016x%016x", g.rng.Uint64(), g.rng.Uint64()))
}

// ClaimIDs satisfies relay.ClaimIDs of app by shape — NewClaimID() string

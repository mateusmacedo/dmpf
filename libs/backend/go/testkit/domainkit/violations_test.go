...
func (l *leaky) Events() []domain.DomainEvent { return l.pending }

func (l *leaky) Bump(limit int) (domain.Accepted[int], *domain.Rejection) {
	ev := counted{Name: "bumped"}
	l.pending = append(l.pending, ev)
	if l.n >= limit {
		return domain.Accepted[int]{}, domain.Reject("test/limit", "limit")
	}
	l.n++
	return domain.Accept(l.n, ev), nil
}

// mutating changes the target and then refuses (row 5 of §3.4, DEC-10).
type mutating struct{ n int }

func (m *mutating) Bump() (domain.Accepted[int], *domain.Rejection) {
	m.n++
	return domain.Accepted[int]{}, domain.Reject("test/refused", "refused")
}

func countSubject[S any](decide func(S) (domain.Accepted[int], *domain.Rejection), snapshot func(S) domainkit.Fields, clone func(S) S) domainkit.Subject[S, int] {
	return domainkit.Subject[S, int]{
		Decide:   decide,
		Response: func(n int) domainkit.Fields { return domainkit.Fields{"n": strconv.Itoa(n)} },
		Event:    func(e domain.DomainEvent) (string, domainkit.Fields) { return e.EventName(), domainkit.Fields{} },
		Snapshot: snapshot,
		Clone:    clone,
	}
}
...
func TestASecondAccessorOfEventsIsReproved(t *testing.T) {
	s := countSubject(
		func(l *leaky) (domain.Accepted[int], *domain.Rejection) { return l.Bump(10) },
...
func TestAPendingCollectionAfterRejectedIsReproved(t *testing.T) {
	s := countSubject(
		func(l *leaky) (domain.Accepted[int], *domain.Rejection) { return l.Bump(0) },
		func(l *leaky) domainkit.Fields { return domainkit.Fields{"n": strconv.Itoa(l.n)} },
		func(l *leaky) *leaky { return &leaky{n: l.n} },
	)
	p := domainkit.Run(&leaky{}, s)
...
func TestAStateMutatedUnderRejectedIsReproved(t *testing.T) {
	s := countSubject(
		func(m *mutating) (domain.Accepted[int], *domain.Rejection) { return m.Bump() },

...
func TestReachListsOnlyWhatTheSubjectImports(t *testing.T) {
	...
	modules, externals, err := evidence.Reach(context.Background(), root, nil,
		[]string{"github.com/mateusmacedo/dmpf/libs/backend/go/contracts/golden"})
	if err != nil {
		t.Fatal(err)
	}
	raw, err := os.ReadFile(filepath.Join(root, "libs/backend/go/contracts/package.json"))
	if err != nil {
		t.Fatal(err)
	}
	...
	contracts := evidence.Module{Path: "github.com/mateusmacedo/dmpf/libs/backend/go/contracts", Version: pkg.Version}

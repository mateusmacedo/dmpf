...
instrumentação que o application service usa vive em `ports`, e é este
...
`bounded_context: kernel`. Doze packages, todos no mesmo `include` do
`dmpf-units.json`:

| Package | Conteúdo |
| --- | --- |
| raiz (`observability`) | `OTelVersion`, `SemconvVersion` — os pins que o `version_test.go` prova contra o grafo real de build |
...
| `usecase` | `Instrumentation`: realiza o gancho de `ports` sobre span, métricas e trilha |

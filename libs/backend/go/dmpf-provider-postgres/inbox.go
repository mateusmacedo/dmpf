...
func (t *Tx) Inbox(consumer string, wait time.Duration) dmpfports.Inbox {
...
func (i *txInbox) Register(ctx context.Context, r dmpfports.Receipt) (dmpfports.Reception, error) {
	if i.consumer == "" {
	...
	if err != nil {
		if isLockTimeout(err) {
			return dmpfports.Reception{}, fmt.Errorf("%w: %w", dmpfports.ErrRegisterTimeout, err)
		}
		return dmpfports.Reception{}, err
	}
	...
	case errors.Is(err, pgx.ErrNoRows):
		return dmpfports.Reception{}, fmt.Errorf("dmpfpostgres: inbox row absent after conflict on (%s, %s)", i.consumer, r.MessageID)
	case err != nil:
		return dmpfports.Reception{}, err
	}
	...
	if storedStatus == dmpfports.StatusProcessed.String() {
...
func (p *pending) Complete(ctx context.Context, c dmpfports.Completion) error {
	...
	if tag.RowsAffected() != 1 {
		return fmt.Errorf("dmpfpostgres: inbox update affected %d rows, want 1", tag.RowsAffected())
	}

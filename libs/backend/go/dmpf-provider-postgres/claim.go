...
func NewOutboxStore(pool *pgxpool.Pool, clock dmpfports.Clock) OutboxStore {
...
func collectClaimed(rows pgx.Rows, claimID string) ([]Claimed, error) {
	...
	for rows.Next() {
		...
		c.OccurredAt = dmpfports.Instant(occurredAt)
...
func (s OutboxStore) Reschedule(ctx context.Context, id int64, claimID string, availableAt dmpfports.Instant, lastError string) (int64, error) {

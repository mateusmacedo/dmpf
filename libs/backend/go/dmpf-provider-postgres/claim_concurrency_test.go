...
func TestALateWriteFromAnExpiredClaimChangesNothing(t *testing.T) {
	...
	first := dmpfpostgres.NewOutboxStore(pool, fixedClock(now))
	...
	afterExpiry := now + dmpfports.Instant(lease) + 1
	second := dmpfpostgres.NewOutboxStore(pool, fixedClock(afterExpiry))

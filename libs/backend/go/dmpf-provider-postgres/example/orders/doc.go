...
// dmpf-kernel/example-orders-postgres unit: the Postgres counterpart of
// dmpf-application/example/memory, over the same orders.Snapshot and the two
// domain events the aggregate produces (OrderPlaced, ItemAdded).
//
// It realizes dmpfports.Repository[orders.OrderID, orders.Snapshot] with
// optimistic locking in SQL and dmpfpostgres.EventMapper into the
// company.orders.event.v1 Protobuf contracts. It never opens its own
// transaction: every query runs on the pgx.Tx a dmpfpostgres.Tx exposes.
package orderspg

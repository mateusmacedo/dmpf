...
// This is what example/memory declares it cannot prove and hands to KRN-06:
// two transactions that really run at the same time, over a real isolation
// level, with the loser told so instead of silently overwriting the winner.
func TestConcurrentSaveLetsExactlyOneWriterThrough(t *testing.T) {
	...
	for i := range writers {
		go func(quantity int) {
			defer running.Done()
			results <- withRepo(t, pool, func(ctx context.Context, repo dmpfports.Repository[orders.OrderID, orders.Snapshot]) error {

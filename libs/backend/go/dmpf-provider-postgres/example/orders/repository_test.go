...
func withRepo(t *testing.T, pool *pgxpool.Pool, fn func(ctx context.Context, repo dmpfports.Repository[orders.OrderID, orders.Snapshot]) error) error {
	t.Helper()
...
func TestSaveCreatesAtVersionOne(t *testing.T) {
	...
	err := withRepo(t, pool, func(ctx context.Context, repo dmpfports.Repository[orders.OrderID, orders.Snapshot]) error {
...
func TestSaveRejectsADivergentExpectedVersion(t *testing.T) {
	...
	if err := withRepo(t, pool, func(ctx context.Context, repo dmpfports.Repository[orders.OrderID, orders.Snapshot]) error {
		return repo.Save(ctx, repoOrderID, snapshot(7), 1)
	}); err != nil {
...
func load(t *testing.T, pool *pgxpool.Pool) (orders.Snapshot, dmpfports.Version) {
	t.Helper()

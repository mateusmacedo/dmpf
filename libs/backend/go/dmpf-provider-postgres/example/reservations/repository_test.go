...
const repoOrderID = reservations.OrderID("order-1")
...
func bindRepo(tx *dmpfpostgres.Tx) repoResources {
	return repoResources{Reservations: reservationspg.NewRepository(tx)}
}

func snapshot(items int) reservations.Snapshot {
...
func withRepo(t *testing.T, pool *pgxpool.Pool, fn func(ctx context.Context, repo dmpfports.Repository[reservations.OrderID, reservations.Snapshot]) error) error {
	t.Helper()
...
func seed(t *testing.T, pool *pgxpool.Pool) {
	t.Helper()
	if err := withRepo(t, pool, func(ctx context.Context, repo dmpfports.Repository[reservations.OrderID, reservations.Snapshot]) error {
...
func load(t *testing.T, pool *pgxpool.Pool) (reservations.Snapshot, dmpfports.Version) {
	t.Helper()
	var (
		s reservations.Snapshot
		v dmpfports.Version
	)
	err := withRepo(t, pool, func(ctx context.Context, repo dmpfports.Repository[reservations.OrderID, reservations.Snapshot]) error {
		var loadErr error
...
func TestConcurrentSaveReservation(t *testing.T) {
	...
	for i := range writers {
		go func(quantity int) {
			defer running.Done()
			results <- withRepo(t, pool, func(ctx context.Context, repo dmpfports.Repository[reservations.OrderID, reservations.Snapshot]) error {

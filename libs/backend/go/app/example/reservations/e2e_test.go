...
type failingReservations struct {
	ports.Repository[reservations.OrderID, reservations.Snapshot]
	err error
}

func (f failingReservations) Save(context.Context, reservations.OrderID, reservations.Snapshot, ports.Version) error {
	return f.err
}
...
func consumerWith(pool *pgxpool.Pool, decorate func(reservationsapp.Resources) reservationsapp.Resources) app.Consumer {
	bind := func(tx *postgres.Tx) reservationsapp.Resources {
...
func TestFirstReceptionAppliesConfirmsAndDerivesTheOutbox(t *testing.T) {
	...
	if outcome.Disposition != application.R1D1 || outcome.Contained {
...
func TestBusinessRejectionCommitsRejectedAndConfirms(t *testing.T) {
	...
	if outcome.Disposition != application.R1D2 || outcome.Contained {
		t.Fatalf("outcome = %+v", outcome)
	}
...
func TestRedeliveriesShortCircuit(t *testing.T) {
	...
	t.Run("R2 reapplies nothing", func(t *testing.T) {
		...
		if err != nil || outcome.Disposition != application.R2 || outcome.Contained {
	...
	t.Run("R3 re-emits no rejection event", func(t *testing.T) {
		...
		if err != nil || outcome.Disposition != application.R3 || outcome.Contained {
			t.Fatalf("outcome = %+v, err = %v", outcome, err)
		}
		if ack.acks != 1 || counts(t, pool) != before {
	...
	t.Run("R4 contains the collision byte for byte", func(t *testing.T) {
		...
		if err != nil || outcome.Disposition != application.R4 || !outcome.Contained || outcome.Reason != ports.ReasonCollision {
			t.Fatalf("outcome = %+v, err = %v", outcome, err)
		}
		if ack.acks != 1 {
...
func TestTransientFailureRollsBackAndReleases(t *testing.T) {
	...
	consumer := consumerWith(pool, func(r reservationsapp.Resources) reservationsapp.Resources {
		r.Reservations = failingReservations{r.Reservations, application.NewFailure(application.TransientDependency, true, errBoom)}
		return r
	})

	outcome, err, ack := consume(t, pool, consumer, "evt-3", appkit.RawOrderPlaced(t, "evt-3", "o-3", 1), 1)
	if !errors.Is(err, errBoom) || outcome.Disposition != application.R1D3 || outcome.Contained {
		t.Fatalf("outcome = %+v, err = %v", outcome, err)
	}
...
func TestExhaustedAttemptsContainThePoisonMessage(t *testing.T) {
	...
	if err != nil || outcome.Disposition != application.R1D1 {
...
func TestTerminalFailureIsContained(t *testing.T) {
	pool := openPool(t)
	consumer := consumerWith(pool, func(r reservationsapp.Resources) reservationsapp.Resources {
		r.Reservations = failingReservations{r.Reservations, application.NewFailure(application.Forbidden, false, errBoom)}
		return r
	})

	outcome, err, ack := consume(t, pool, consumer, "evt-6", appkit.RawOrderPlaced(t, "evt-6", "o-6", 1), 1)
	if !errors.Is(err, errBoom) || outcome.Disposition != application.R1D4 || outcome.Reason != ports.ReasonTerminalFailure {
		t.Fatalf("outcome = %+v, err = %v", outcome, err)
	}
...
func TestPayloadOfAnotherContractIsTerminal(t *testing.T) {
	...
	if !errors.Is(err, envelope.ErrSchemaMismatch) || outcome.Disposition != application.R1D4 || outcome.Reason != ports.ReasonTerminalFailure {
		t.Fatalf("outcome = %+v, err = %v", outcome, err)
	}
	if got := counts(t, pool); got != (tableCounts{quarantine: 1}) {
...
func TestSignalsExposeTheConsumerSide(t *testing.T) {
	...
	signals, err := postgres.InboxSignals(context.Background(), pool, reservationsconsumer.ConsumerName)
	if err != nil {

...
func (r Relay) terminal(ctx context.Context, record postgres.Claimed, reason string) (disposition, error) {
	settleCtx, cancel := r.settleContext(ctx)
	defer cancel()
...
func (r Relay) reschedule(ctx context.Context, record postgres.Claimed, reason string) (disposition, error) {
	settleCtx, cancel := r.settleContext(ctx)
	defer cancel()

	availableAt := r.Clock.Now() + ports.Instant(r.backoffFor(record.AttemptCount))

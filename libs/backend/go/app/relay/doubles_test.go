...
func newFakeStore(batches ...[]postgres.Claimed) *fakeStore {
...
func (s *fakeStore) Reschedule(ctx context.Context, id int64, claimID string, availableAt ports.Instant, lastError string) (int64, error) {
...
func (s *fakeStore) OutboxSignals(context.Context) (postgres.OutboxHealth, error) {
	return s.health, nil
}
...
func batchOf(t testingT, n int) []postgres.Claimed {
	t.Helper()

	batch := make([]postgres.Claimed, 0, n)
	for i := range n {

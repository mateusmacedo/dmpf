...
// here, in the consumer, and satisfied structurally by postgres.OutboxStore.
type Store interface {
	// Claim acquires up to limit eligible records under claimID, writing the
	// four fields of OBX-16 in one commit.
	Claim(ctx context.Context, claimID string, limit int, lease time.Duration) ([]postgres.Claimed, error)
	...
	Reschedule(ctx context.Context, id int64, claimID string, availableAt ports.Instant, lastError string) (int64, error)
	...
	OutboxSignals(ctx context.Context) (postgres.OutboxHealth, error)
}

// Publisher hands the serialized envelope to a transport. It is declared here
// rather than in ports because publishing happens after the commit and

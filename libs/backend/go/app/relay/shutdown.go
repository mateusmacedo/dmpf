...
type held struct {
	mu      sync.Mutex
	records map[int64]postgres.Claimed
}

func newHeld() *held { return &held{records: make(map[int64]postgres.Claimed)} }

func (h *held) take(record postgres.Claimed) {
	h.mu.Lock()
	defer h.mu.Unlock()
...
func (h *held) remaining() []postgres.Claimed {
	h.mu.Lock()
	defer h.mu.Unlock()

	remaining := make([]postgres.Claimed, 0, len(h.records))
...
func (r Relay) release(ctx context.Context, records []postgres.Claimed) {
	if len(records) == 0 {
		return
	}

...
var (
	...
	ErrRouteNotDeclared = errors.New("http: route is not declared")
	...
	ErrMethodMismatch = errors.New("http: request method differs from the route's")
	...
	ErrBodyNotReplayable = errors.New("http: request body has no GetBody and the route may retry")

	// ErrIncompleteConfig is a configuration without routes or clock.
	ErrIncompleteConfig = errors.New("http: configuration is incomplete")
)
...
func (c *Client) attempt(ctx context.Context, req *http.Request, hasBody bool) (*http.Response, error) {
	...
	if hasBody && req.GetBody != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("http: GetBody: %w", err)
		}
		try.Body = body
	}
...
func randomKey() string {
	...
	if _, err := cryptorand.Read(b[:]); err != nil {
		panic(fmt.Sprintf("http: crypto/rand: %v", err))
	}

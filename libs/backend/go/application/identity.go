...
func ResolveIdentity(clock ports.Clock, ids ports.IDGenerator, events int) Identity {
	if events < 0 {
		panic("application: ResolveIdentity requires a non-negative event count")
	}

	identity := Identity{OccurredAt: clock.Now(), MessageIDs: make([]ports.MessageID, 0, events)}
	for range events {
		next := ids.NewMessageID()
		if next == "" {
			panic("application: IDGenerator produced an empty MessageID")
		}
		for _, taken := range identity.MessageIDs {
			if taken == next {
				panic("application: IDGenerator repeated a MessageID within one resolution")
			}
		}

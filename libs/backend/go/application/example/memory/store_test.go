...
func entry(id ports.MessageID) ports.OutboxEntry {
	return ports.OutboxEntry{
		...
		Intent:           ports.PublishIntent{Destination: "orders.events", PartitionKey: string(orderID)},
...
func seed(t *testing.T, store *memory.Store, snapshot orders.Snapshot, expected ports.Version) {
	t.Helper()
...
func requireEmpty(t *testing.T, store *memory.Store) {
	t.Helper()
	if _, _, err := store.Reader().Load(context.Background(), orderID); !errors.Is(err, ports.ErrNotFound) {

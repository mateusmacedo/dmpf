...
func (s *Store) Reader() ports.Reader[orders.OrderID, orders.Snapshot] {
	return storeReader{store: s}
}

// ReservationsReader mirrors Reader for the reservations aggregate.
func (s *Store) ReservationsReader() ports.Reader[reservations.OrderID, reservations.Snapshot] {
...
func (r storeReader) Load(_ context.Context, id orders.OrderID) (orders.Snapshot, ports.Version, error) {
	r.store.dataMu.Lock()
	defer r.store.dataMu.Unlock()
	return load(r.store.orders, id)
}

func load(from map[orders.OrderID]record, id orders.OrderID) (orders.Snapshot, ports.Version, error) {
	rec, ok := from[id]
	if !ok {
		return orders.Snapshot{}, 0, ports.ErrNotFound
	}
...
func (r reservationsStoreReader) Load(_ context.Context, id reservations.OrderID) (reservations.Snapshot, ports.Version, error) {
	r.store.dataMu.Lock()
	defer r.store.dataMu.Unlock()
	return loadReservation(r.store.reservations, id)
}

func loadReservation(from map[reservations.OrderID]reservationRecord, id reservations.OrderID) (reservations.Snapshot, ports.Version, error) {
	rec, ok := from[id]
	if !ok {
		return reservations.Snapshot{}, 0, ports.ErrNotFound
	}

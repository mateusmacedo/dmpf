...
type syncRepository struct {
	inner   ports.Repository[reservations.OrderID, reservations.Snapshot]
	h       *syncHarness
	saveErr error
}

func (r syncRepository) Load(ctx context.Context, id reservations.OrderID) (reservations.Snapshot, ports.Version, error) {
	r.h.rec.record("reservations.Load")
	return r.inner.Load(ctx, id)
}

func (r syncRepository) Save(ctx context.Context, id reservations.OrderID, state reservations.Snapshot, expected ports.Version) error {
...
type syncUnitOfWork struct {
	inner ports.UnitOfWork[reservationsapp.Resources]
	rec   *syncRecorder
}
...
type syncSetup struct {
	saveErr   error
	authorize application.AuthorizeFunc[reservationsapp.Command]
}
...
func withSyncAuthorize(authorize application.AuthorizeFunc[reservationsapp.Command]) syncOption {
...
func newSyncHarness(t *testing.T, options ...syncOption) *syncHarness {
	...
	cfg := &syncSetup{authorize: application.AllowAll[reservationsapp.Command]()}
...
func (h *syncHarness) seed(t *testing.T, snapshot reservations.Snapshot, expected ports.Version) {
	t.Helper()

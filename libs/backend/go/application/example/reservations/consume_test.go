...
func consumeOrderPlaced(id ports.MessageID, hash string, order reservations.OrderID, items int) reservationsapp.ConsumeOrderPlaced {
...
func TestConsumeFirstReceptionAppliesAndConfirms(t *testing.T) {
	...
	if disp != application.R1D1 {
		t.Fatalf("Consume() disposition = %v, want %v", disp, application.R1D1)
	}
...
func TestConsumeZeroItemsRejects(t *testing.T) {
	...
	if disp != application.R1D2 {
		t.Fatalf("Consume() disposition = %v, want %v", disp, application.R1D2)
	}
	status, ok := store.InboxStatus(consumer, "m-ext-1")
	...
	if _, _, err := store.ReservationsReader().Load(context.Background(), "P-100"); !errors.Is(err, ports.ErrNotFound) {
...
func TestConsumeATransientSaveFailureIsD3AndPersistsNothing(t *testing.T) {
	store := memory.New()
	failure := application.NewFailure(application.TransientDependency, true, errors.New("consume_test: dependency down"))
	...
	if disp != application.R1D3 {
		t.Fatalf("Consume() disposition = %v, want %v", disp, application.R1D3)
	}
...
func TestConsumeATerminalSaveFailureIsD4AndPersistsNothing(t *testing.T) {
	store := memory.New()
	failure := application.NewFailure(application.Unexpected, false, errors.New("consume_test: boom"))
	svc := newServiceWithFailingSave(store, failure)
	...
	if disp != application.R1D4 {
		t.Fatalf("Consume() disposition = %v, want %v", disp, application.R1D4)
	}
	requireNothingPersisted(t, store)
}
...
func TestConsumeRedeliveryOfAProcessedMessageIsR2(t *testing.T) {
	...
	if disp != application.R2 {
		t.Fatalf("Consume() disposition = %v, want %v", disp, application.R2)
	}
...
func TestConsumeRedeliveryOfARejectedMessageIsR3(t *testing.T) {
	...
	if disp != application.R3 {
		t.Fatalf("Consume() disposition = %v, want %v", disp, application.R3)
	}
	if got := len(store.Entries()); got != 0 {
...
func TestConsumeADivergentHashOnAPresentKeyIsR4(t *testing.T) {
	...
	if disp != application.R4 {
		t.Fatalf("Consume() disposition = %v, want %v", disp, application.R4)
	}
	if got := len(store.Entries()); got != 1 {
...
func TestConsumeAFailedCommitClassifiesToR1D4AndPersistsNothing(t *testing.T) {
	...
	if disp != application.R1D4 {
		t.Fatalf("Consume() disposition = %v, want %v (INB-07)", disp, application.R1D4)
	}
	requireNothingPersisted(t, store)
}
...
func TestConsumeTwoMessagesForTheSameOrderReserveOnlyOnce(t *testing.T) {
	...
	if disp != application.R1D2 {
		t.Fatalf("second Consume() disposition = %v, want %v (GAR-10)", disp, application.R1D2)
	}
...
func TestConsumeAuthorizeDenyingNeverOpensATransaction(t *testing.T) {
	...
	if disposition != application.R1D4 {
		t.Fatalf("disposition = %v, want %v", disposition, application.R1D4)
	}
...
func TestConsumeAuthorizeDenyingWithACategoryKeepsIt(t *testing.T) {
	...
	svc := newService(store)
	svc.Authorize = func(context.Context, reservationsapp.Command) error {
		return application.NewFailure(application.Forbidden, false, errors.New("consume_test: forbidden"))
	}

	disposition, err := svc.Consume(context.Background(), consumeOrderPlaced("m-ext-2", "h1", "P-100", 3))

	var failure *application.Failure
	if !errors.As(err, &failure) || failure.Category() != application.Forbidden {
		t.Fatalf("err = %v, want the Forbidden failure", err)
	}
	if disposition != application.R1D4 {
		t.Fatalf("disposition = %v, want %v (FND-07 §6.2: Forbidden is terminal)", disposition, application.R1D4)
	}
}

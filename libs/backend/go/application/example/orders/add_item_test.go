...
func TestAddItemCreatesTheAggregateAndAuthorsTheOutboxEntry(t *testing.T) {
	...
	want := ports.OutboxEntry{
		MessageID:        "m-000001",
		OccurredAt:       occurred,
		Intent:           ports.PublishIntent{Destination: ordersapp.Destination, PartitionKey: string(orderID)},
...
func TestAddItemKeepsNothingWhenTheCommitFailsWhileCreating(t *testing.T) {
	...
	if _, _, err := h.store.Reader().Load(context.Background(), orderID); !errors.Is(err, ports.ErrNotFound) {
...
func TestWritingIntoAnUnboundResourceEscapesTheTransaction(t *testing.T) {
	...
	if _, _, err := a.Reader().Load(context.Background(), orderID); !errors.Is(err, ports.ErrNotFound) {

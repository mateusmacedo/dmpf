...
func (s Service) AddItem(ctx context.Context, cmd AddItem) (application.Outcome[orders.ItemAccepted], error) {
	var zero application.Outcome[orders.ItemAccepted]
	...
	err := s.UoW.Within(ctx, func(ctx context.Context, res Resources) error {
		...
		if rejection != nil {
			...
			outcome = application.Rejected[orders.ItemAccepted](rejection)
			return nil
		}
...
func (s Service) loadOrCreate(ctx context.Context, res Resources, id orders.OrderID) (*orders.Order, ports.Version, error) {

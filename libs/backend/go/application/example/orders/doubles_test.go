...
type recordingRepository struct {
	inner   ports.Repository[orders.OrderID, orders.Snapshot]
	h       *harness
	saveErr error
}

func (r recordingRepository) Load(ctx context.Context, id orders.OrderID) (orders.Snapshot, ports.Version, error) {
	r.h.rec.record("orders.Load")
	r.h.loads++
	return r.inner.Load(ctx, id)
}

func (r recordingRepository) Save(ctx context.Context, id orders.OrderID, state orders.Snapshot, expected ports.Version) error {
...
type setup struct {
	saveErr   error
	authorize application.AuthorizeFunc[ordersapp.Command]
}
...
func withAuthorize(authorize application.AuthorizeFunc[ordersapp.Command]) option {
...
func newHarness(t *testing.T, options ...option) *harness {
	...
	cfg := &setup{authorize: application.AllowAll[ordersapp.Command]()}
...
func recordingAuthorize(rec *recorder, inner application.AuthorizeFunc[ordersapp.Command]) application.AuthorizeFunc[ordersapp.Command] {
...
func (h *harness) seed(t *testing.T, snapshot orders.Snapshot, expected ports.Version) {
	t.Helper()

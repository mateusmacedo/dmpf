...
import (
	...
	"gitea.lidercap.com.br/lidercap-apps/lidercap-platform/libs/backend/go/dmpf-testkit/serviceskit"
	"gitea.lidercap.com.br/lidercap-apps/lidercap-platform/libs/backend/go/dmpf-testkit/tb"
)

const at = dmpfports.Instant(1_755_432_000_000_000_000)
...
type uowOf[R any] struct{ inner dmpfports.UnitOfWork[any] }
...
func TestEnqueueBeforeBeginIsNamedWithItsPosition(t *testing.T) {
	...
	f.Ledger.Reset()
	if err := stray.Enqueue(ctx, dmpfports.OutboxEntry{MessageID: "m-stray", OccurredAt: 1, Intent: dmpfports.PublishIntent{Destination: "x", PartitionKey: "k"}, AggregateType: "t", AggregateID: "a"}); err != nil {
		t.Fatal(err)
	}
	if err := uow.Within(ctx, func(context.Context, any) error { return nil }); err != nil {
		t.Fatal(err)

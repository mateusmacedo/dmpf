...
}, consumer string, fn func(ctx context.Context, inbox ports.Inbox) error) error {
	t.Helper()
...
func receipt(consumer, id, hash string) ports.Receipt {
	return ports.Receipt{
		Consumer:    consumer,
		MessageID:   ports.MessageID(id),
		MessageType: "example",
		PayloadHash: hash,
		ReceivedAt:  100,
	}
...
func matchBranch(t *testing.T, r ports.Reception, completeAs *ports.Status) string {
	t.Helper()
	var branch string
	completion := ports.Completion{Status: ports.StatusProcessed, At: 200}
	if completeAs != nil {
...
func TestInboxFirstReception(t *testing.T) {
	...
	err := withInbox(t, pool, "orders", func(ctx context.Context, inbox ports.Inbox) error {
		...
		if branch := matchBranch(t, reception, statusPtr(ports.StatusProcessed)); branch != "first" {
...
func TestInboxFirstAgainAfterRollback(t *testing.T) {
	...
	err = withInbox(t, pool, "orders", func(ctx context.Context, inbox ports.Inbox) error {
		...
		if branch := matchBranch(t, r, statusPtr(ports.StatusProcessed)); branch != "first" {
			t.Fatalf("branch = %q, want %q", branch, "first")
		}
		return nil
	})
...
func TestInboxDistinctConsumers(t *testing.T) {
	...
	err = withInbox(t, pool, "billing", func(ctx context.Context, inbox ports.Inbox) error {
...
func TestInboxCompleteTwice(t *testing.T) {
	...
	err := withInbox(t, pool, "orders", func(ctx context.Context, inbox ports.Inbox) error {
		...
		_ = r.Match(
			func(p ports.Pending) error {
				if err := p.Complete(ctx, ports.Completion{Status: ports.StatusProcessed, At: 200}); err != nil {
					return err
				}
				completionErr = p.Complete(ctx, ports.Completion{Status: ports.StatusProcessed, At: 300})
				return nil
			},
		...
		if !errors.Is(completionErr, postgres.ErrAlreadyCompleted) {
...
func TestInboxLastErrorStoredOnRejected(t *testing.T) {
	...
	_ = r.Match(
		func(p ports.Pending) error {
			return p.Complete(ctx, ports.Completion{
...
func TestRegisterRestoresLockTimeoutForTheRestOfTheTransaction(t *testing.T) {
	...
	inbox := postgres.NewTx(pgxTx).Inbox("orders", 300*time.Millisecond)
...
func TestCompleteRefusesAStatusOutsideTheTwoTerminalValues(t *testing.T) {
	...
	err := withInbox(t, pool, "orders", func(ctx context.Context, inbox ports.Inbox) error {
		...
		return r.Match(
			func(p ports.Pending) error { return p.Complete(ctx, ports.Completion{At: 200}) },
			func() error { return nil }, func() error { return nil }, func() error { return nil },
		)
	})
	if !errors.Is(err, postgres.ErrInvalidCompletion) {

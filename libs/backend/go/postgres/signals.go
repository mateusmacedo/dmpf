...
func InboxSignals(ctx context.Context, pool *pgxpool.Pool, consumer string) (Signals, error) {
	...
	for rows.Next() {
		...
		switch ports.Reason(reason) {
		case ports.ReasonTerminalFailure:
			s.TerminalFailures = count
		case ports.ReasonCollision:
			s.Collisions = count
		case ports.ReasonAttemptsExhausted:
...
func OutboxSignals(ctx context.Context, pool *pgxpool.Pool, clock ports.Clock) (OutboxHealth, error) {

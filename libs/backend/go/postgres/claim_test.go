...
type fixedClock ports.Instant

func (c fixedClock) Now() ports.Instant { return ports.Instant(c) }
...
func TestClaimSelectsWhatEnqueueWroteWithAMessageContext(t *testing.T) {
	...
	store := postgres.NewOutboxStore(pool, systemClock{})
...
func TestClaimNeverLeavesPublishingWithoutALease(t *testing.T) {
	...
	store := postgres.NewOutboxStore(pool, fixedClock(10_000))
...
func TestReclaimingTheSameRecordProducesADifferentClaimIdentity(t *testing.T) {
	...
	expired := postgres.NewOutboxStore(pool, fixedClock(now+ports.Instant(testLease)+1))
...
func claimOne(t *testing.T, ctx context.Context, pool *pgxpool.Pool, now ports.Instant, claimID string) postgres.Claimed {
	t.Helper()
...
func TestBackoffShorterThanTheLeaseGovernsTheNextClaim(t *testing.T) {
	...
	ctx := context.Background()
	now := ports.Instant(10_000)
	const backoff = ports.Instant(1_000)
	...
	tooEarly := postgres.NewOutboxStore(pool, fixedClock(now+backoff-1))
	...
	onTime := postgres.NewOutboxStore(pool, fixedClock(now+backoff))
...
func TestRescheduleNeverWritesAvailableAtBeforeOccurredAt(t *testing.T) {
	...
	ahead := postgres.NewOutboxStore(pool, fixedClock(now+60_000))
	claimed, err := ahead.Claim(ctx, claimA, 10, testLease)
	if err != nil || len(claimed) != 1 {
		t.Fatalf("Claim() = %v, %d rows; want nil, 1", err, len(claimed))
	}

	behind := postgres.NewOutboxStore(pool, fixedClock(now))

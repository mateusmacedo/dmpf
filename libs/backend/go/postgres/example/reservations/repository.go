...
func NewRepository(tx *postgres.Tx) ports.Repository[reservations.OrderID, reservations.Snapshot] {
	return repository{conn: tx.Conn()}
}

type repository struct{ conn pgx.Tx }

func (r repository) Load(ctx context.Context, id reservations.OrderID) (reservations.Snapshot, ports.Version, error) {
...
func load(ctx context.Context, q querier, id reservations.OrderID) (reservations.Snapshot, ports.Version, error) {
	var (
		version int64
		raw     []byte
	)
...
func (r repository) Save(ctx context.Context, id reservations.OrderID, state reservations.Snapshot, expected ports.Version) error {

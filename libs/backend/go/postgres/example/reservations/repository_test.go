...
func withRepo(t *testing.T, pool *pgxpool.Pool, fn func(ctx context.Context, repo ports.Repository[reservations.OrderID, reservations.Snapshot]) error) error {
	t.Helper()
...
func seed(t *testing.T, pool *pgxpool.Pool) {
	t.Helper()
	if err := withRepo(t, pool, func(ctx context.Context, repo ports.Repository[reservations.OrderID, reservations.Snapshot]) error {
...
func load(t *testing.T, pool *pgxpool.Pool) (reservations.Snapshot, ports.Version) {
	t.Helper()
	var (
		s reservations.Snapshot
		v ports.Version
	)
	err := withRepo(t, pool, func(ctx context.Context, repo ports.Repository[reservations.OrderID, reservations.Snapshot]) error {
		var loadErr error
...
func TestConcurrentSaveReservation(t *testing.T) {
	...
	for i := range writers {
		go func(quantity int) {
			defer running.Done()
			results <- withRepo(t, pool, func(ctx context.Context, repo ports.Repository[reservations.OrderID, reservations.Snapshot]) error {

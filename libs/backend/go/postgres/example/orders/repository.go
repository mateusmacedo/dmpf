...
func NewRepository(tx *postgres.Tx) ports.Repository[orders.OrderID, orders.Snapshot] {
	return repository{conn: tx.Conn()}
}

type repository struct{ conn pgx.Tx }

func (r repository) Load(ctx context.Context, id orders.OrderID) (orders.Snapshot, ports.Version, error) {
...
func load(ctx context.Context, q querier, id orders.OrderID) (orders.Snapshot, ports.Version, error) {
	var (
		version int64
		raw     []byte
	)
...
func (r repository) Save(ctx context.Context, id orders.OrderID, state orders.Snapshot, expected ports.Version) error {

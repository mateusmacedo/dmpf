...
func (s *recordingSink) Handle(ctx context.Context, raw []byte, _ int, ack ports.Acknowledger) error {
...
func integrationConfig(seeds []string, ch channel.Channel) kafka.Config {
	cfg := publishConfig()
	cfg.Brokers = seeds
...
func runConsumer(t *testing.T, cfg kafka.Config, ch channel.Channel, sink *recordingSink) (context.CancelFunc, <-chan error) {
	t.Helper()
	c := &kafka.Consumer{

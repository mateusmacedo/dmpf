---
name: fill-pr-template-from-diff
description: 'Preenche template de Pull Request com base no diff da branch atual versus uma branch base (padrão: origin/develop). USE WHEN: (1) o usuário pedir para preencher descrição de PR automaticamente, (2) houver um template de PR a ser completado, (3) for necessário classificar tipo de PR (Feature, Bug Fix, CI, Build etc.) com base em commits e arquivos alterados. DO NOT USE WHEN: (a) o usuário quiser apenas explicação teórica sem geração de texto final, (b) não houver acesso ao histórico Git/diff necessário para inferência.'
---
...
Transformar diff, commits e estatísticas de mudança em um template de PR preenchido, claro e consistente.
...
"Compare a branch atual com `${baseRef}` (default `origin/develop`) e preencha o template de PR fornecido.
Use apenas evidências do diff e commits. Classifique o tipo de PR por heurísticas de arquivos e mensagens.
Mantenha PT-BR, objetividade e consistência. Não invente dados ausentes."

## Critérios de aceite da skill

- O template é preenchido integralmente
- As marcações de tipo de PR são justificáveis pelo diff
- O checklist é conservador e sem suposições
- O resultado final está pronto para uso em PR real

...
A borda não autentica ninguém (a identidade de FND-07 não tem realização no kernel), então quem a alcança é decidido pela rede: a `NetworkPolicy` da base só admite ingress na api de pods rotulados `dmpf/api-client: "true"`, e nega todo ingress a `relay` e `consumer`. O Alloy roda como DaemonSet e lê os logs dos pods pela API do kubelet, com RBAC de `pods`, `pods/log` e `namespaces` — sem `hostPath` e sem `nodes/proxy`.

...
func NewRelay(pool *pgxpool.Pool, publisher relay.Publisher, cfg relay.Config) (relay.Relay, error) {
	clock := SystemClock{}
	return relay.New(postgres.NewOutboxStore(pool, clock), publisher, RandomClaimIDs{}, clock, cfg)
}
...
func runRelay(ctx context.Context, cfg Config, rt *otelboot.Runtime) error {
	...
	publisher, err := kafka.NewPublisher(NewKafkaConfig(ctx, cfg, rt, catalog), nil)
	if err != nil {
		return err
	}

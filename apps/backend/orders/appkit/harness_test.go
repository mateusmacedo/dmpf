...
func TestTheUseCaseLeavesOneOutboxRecordPerDecision(t *testing.T) {
	...
	added, err := h.Service.AddItem(ctx, testExecution(t), application.AddItem{Order: orderUnderTest, SKU: "sku-1", Quantity: 2})
	if err != nil {
	...
	placed, err := h.Service.PlaceOrder(ctx, testExecution(t), application.PlaceOrder{Order: orderUnderTest})
	if err != nil {
...
func TestARejectedDecisionLeavesNothingBehind(t *testing.T) {
	h := harness(t)
	ctx := context.Background()
	if _, err := h.Service.AddItem(ctx, testExecution(t), application.AddItem{Order: orderUnderTest, SKU: "sku-1", Quantity: 1}); err != nil {
		t.Fatalf("AddItem() = %v, want nil", err)
	}
	if _, err := h.Service.PlaceOrder(ctx, testExecution(t), application.PlaceOrder{Order: orderUnderTest}); err != nil {
		t.Fatalf("PlaceOrder() = %v, want nil", err)
	}
	...
	again, err := h.Service.PlaceOrder(ctx, testExecution(t), application.PlaceOrder{Order: orderUnderTest})

...
// Duplicated from the postgres module's harness on purpose: a _test.go file is
// never importable, so provider_test cannot reuse postgres_test's openPool.
// An exported test kit is KRN-11's.
...
func truncate(t *testing.T, pool *pgxpool.Pool) {
	...
	if _, err := pool.Exec(context.Background(), "TRUNCATE dmpf_outbox, dmpf_example_orders"); err != nil {

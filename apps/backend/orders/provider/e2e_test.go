...
func TestTheUseCaseRunsEndToEndOverPostgres(t *testing.T) {
	...
	added, err := service.AddItem(ctx, testExecution(t), application.AddItem{Order: repoOrderID, SKU: "sku-1", Quantity: 2})
	if err != nil {
	...
	placed, err := service.PlaceOrder(ctx, testExecution(t), application.PlaceOrder{Order: repoOrderID})
	if err != nil {
	...
	t.Run("a rejection commits nothing and is not an error", func(t *testing.T) {
		...
		outcome, err := service.AddItem(ctx, testExecution(t), application.AddItem{Order: repoOrderID, SKU: "sku-2", Quantity: 1})

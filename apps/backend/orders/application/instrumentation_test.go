...
func (r *instrumentationRecorder) onlyResult(t *testing.T) ports.Result {
	t.Helper()
	if len(r.results) != 1 {
...
func TestAddItemAcceptedReportsAcceptedAndAudits(t *testing.T) {
	...
	_, err := h.service.AddItem(withExecution(t, context.Background()), application.AddItem{Order: orderID, SKU: "XYZ", Quantity: 1})

	if err != nil {
		t.Fatalf("AddItem() error = %v, want nil", err)
	}
	if got := instr.begins; len(got) != 1 || got[0] != application.OperationAddItem {
		t.Fatalf("BeginOperation operations = %v, want [%q]", got, application.OperationAddItem)
	}
	if got := instr.onlyResult(t); got.Outcome != ports.OutcomeAccepted || got.Err != nil {
	...
	want := ports.AuditEvent{
		Object:  string(orderID),
		Action:  application.OperationAddItem,
...
func TestAddItemRejectedReportsRejectedAndAudits(t *testing.T) {
	...
	if got := instr.onlyResult(t); got.Outcome != ports.OutcomeRejected || got.Err != nil {
...
func TestAddItemDeniedReportsDeniedWithoutAuditOrTransaction(t *testing.T) {
	denied := func(context.Context, application.Operation) error {
	...
	if got.Outcome != ports.OutcomeDenied {
...
func TestAddItemAuthorizerFailureReportsFailedNotDenied(t *testing.T) {
	...
	h, instr := newInstrumentedHarness(t, withAuthorize(func(context.Context, application.Operation) error {
		return broken
	}))
	...
	got := instr.onlyResult(t)
	if got.Outcome != ports.OutcomeFailed {
...
func TestPlaceOrderAcceptedReportsAcceptedAndAudits(t *testing.T) {
	...
	if got := instr.begins; len(got) != 1 || got[0] != application.OperationPlaceOrder {
		t.Fatalf("BeginOperation operations = %v, want [%q]", got, application.OperationPlaceOrder)
	}
	if got := instr.onlyResult(t); got.Outcome != ports.OutcomeAccepted {
		t.Fatalf("Result = %+v, want accepted", got)
	}
	want := ports.AuditEvent{
		Object:  string(orderID),
		Action:  application.OperationPlaceOrder,
		Outcome: ports.OutcomeAccepted,
		At:      occurred,
	}
...
func TestFindOrderReportsAcceptedWithoutAudit(t *testing.T) {
	...
	if _, err := h.service.FindOrder(withExecution(t, context.Background()), orderID); err != nil {
		t.Fatalf("FindOrder() error = %v, want nil", err)
	}

	if got := instr.begins; len(got) != 1 || got[0] != application.OperationFindOrder {
		t.Fatalf("BeginOperation operations = %v, want [%q]", got, application.OperationFindOrder)
	}
	if got := instr.onlyResult(t); got.Outcome != ports.OutcomeAccepted {
		t.Fatalf("Result = %+v, want accepted", got)
	}
...
func TestFindOrderOnAnAbsentAggregateReportsFailed(t *testing.T) {
	...
	if got := instr.onlyResult(t); got.Outcome != ports.OutcomeFailed || got.Err == nil {
...
func TestBeginPrecedesAuthorizationAndEndClosesTheSequence(t *testing.T) {
	...
	if _, err := h.service.AddItem(withExecution(t, context.Background()), application.AddItem{Order: orderID, SKU: "XYZ", Quantity: 1}); err != nil {
		t.Fatalf("AddItem() error = %v, want nil", err)
	}

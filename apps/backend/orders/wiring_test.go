...
func TestServeGRPCServesOnlyAfterReadyAndStopsServingOnShutdown(t *testing.T) {
	server, healthServer, err := grpc.NewServer(grpc.ServerConfig{InsecureForDevelopmentOnly: true, Services: []string{rpc.ServiceName}})
	if err != nil {
...
func TestHealthServicesCoverTheOverallStatus(t *testing.T) {
	...
	server, healthServer, err := grpc.NewServer(grpc.ServerConfig{InsecureForDevelopmentOnly: true, Services: services})
	if err != nil {
		t.Fatalf("NewServer() = %v", err)
	}
	defer server.Stop()
...
func TestTheReadinessLineCarriesThePortTheKernelChose(t *testing.T) {
	...
	server, healthServer, err := grpc.NewServer(grpc.ServerConfig{InsecureForDevelopmentOnly: true, Services: healthServices()})
	if err != nil {
		t.Fatalf("NewServer() = %v", err)
	}
	ctx, cancel := context.WithCancel(context.Background())

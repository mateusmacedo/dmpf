...
| `relay` | Drena a outbox de `dmpf_orders` para `orders.events`, autenticado no broker | `app/relay` → `postgres` (claim) + `kafka` (publisher) |
...
| `provider` | `provider` | `orders/provider-postgres` | Repositório escopado por `tenant_id` via `postgres.Table` (ADR-051) sobre `dmpf_example_orders`, `Reader`, mapeador para `company.orders.event.v1` |

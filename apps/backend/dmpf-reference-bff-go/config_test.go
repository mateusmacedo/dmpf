...
func TestFromEnvAppliesTheDefaults(t *testing.T) {
	cfg, err := dmpfreferencebff.FromEnv(lookup(append(targets, "DMPF_GRPC_INSECURE", "true")...))

	if err != nil {
		t.Fatalf("FromEnv() = %v", err)
	}
	if cfg.HTTPAddr != ":8080" || cfg.Service != "dmpf-reference-bff" || !cfg.GRPCInsecure {
		t.Fatalf("cfg = %+v, want :8080, dmpf-reference-bff and the opt-out", cfg)
	}
...
func TestFromEnvNamesEachMissingTarget(t *testing.T) {
	for _, variable := range []string{"DMPF_ORDERS_GRPC_TARGET", "DMPF_RESERVATIONS_GRPC_TARGET"} {
		t.Run(variable, func(t *testing.T) {
			...
			_, err := dmpfreferencebff.FromEnv(lookup(env...))
			if !errors.Is(err, dmpfreferencebff.ErrMissingVariable) || !strings.Contains(err.Error(), variable) {
...
func TestFromEnvRequiresATransportPolicy(t *testing.T) {
	_, err := dmpfreferencebff.FromEnv(lookup(targets...))

	if !errors.Is(err, dmpfreferencebff.ErrMissingVariable) || !strings.Contains(err.Error(), "DMPF_GRPC_INSECURE") || !strings.Contains(err.Error(), "DMPF_GRPC_CA_FILE") {
		t.Fatalf("FromEnv() = %v, want both transport variables named (GRP-15)", err)
	}
}

func TestFromEnvAcceptsATrustAuthority(t *testing.T) {
	cfg, err := dmpfreferencebff.FromEnv(lookup(append(targets, "DMPF_GRPC_CA_FILE", "/etc/ca.pem", "DMPF_GRPC_SERVER_NAME", "orders.internal", "DMPF_CORS_ORIGINS", "http://a, http://b")...))
...
func TestFromEnvRefusesAnInvalidBoolean(t *testing.T) {
	_, err := dmpfreferencebff.FromEnv(lookup(append(targets, "DMPF_GRPC_INSECURE", "maybe")...))

	if !errors.Is(err, dmpfreferencebff.ErrInvalidVariable) {

...
```mermaid
flowchart LR
  client[cliente HTTP] --> bff[dmpf-reference-bff-go]
  bff -->|gRPC| orders[dmpf-reference-orders-go api]
  bff -->|gRPC| reservations[dmpf-reference-reservations-go api]
...
`dmpf-reference-bff/app`, bloco `app`, `bounded_context` `dmpf-reference-bff`. O BFF alcança só o contrato gerado de `company.{orders,reservations}.service.v1` — superfície pública por construção — e o shared kernel: `dmpf-provider-http`, `dmpf-provider-grpc`, `dmpf-transport`, `dmpf-observability` e `dmpf-ports`. Importar domínio ou aplicação dos exemplos reprova no verificador com `DMPF-D002`; o `external` não declara `pgx` nem `franz-go`.

## Cadeia de uma requisição

1. `dmpfhttp.Admission` decide antes de ler o corpo (`RES-17`); recusa com 429.
...
DMPF_ORDERS_GRPC_TARGET=dns:///localhost:9090 DMPF_RESERVATIONS_GRPC_TARGET=dns:///localhost:9091 \
  DMPF_GRPC_INSECURE=true pnpm nx run dmpf-reference-bff-go:serve
```
...
DMPF_PG_DSN='postgres://app:app@localhost:5432/app?sslmode=disable' DMPF_KAFKA_BROKERS=localhost:9092 \
  pnpm nx run dmpf-reference-bff-go:test-race
```

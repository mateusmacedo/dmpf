...
func TestTheReadinessLineCarriesThePortTheKernelChose(t *testing.T) {
	cfg, err := dmpfreferencebff.FromEnv(lookup(append(targets, "DMPF_GRPC_INSECURE", "true", "DMPF_HTTP_ADDR", "127.0.0.1:0")...))
	if err != nil {
	...
	defer cancel()
	runtime, err := dmpfreferencebff.NewTelemetry(ctx, cfg, &logs)
	if err != nil {
	...
	go func() { done <- dmpfreferencebff.RunWith(ctx, cfg, runtime) }()

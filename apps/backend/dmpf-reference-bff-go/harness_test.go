...
func buildBinaries(t *testing.T) binaries {
	...
	dir := t.TempDir()
	out := binaries{
		bff:          filepath.Join(dir, "dmpf-reference-bff"),
		orders:       filepath.Join(dir, "dmpf-reference-orders"),
		reservations: filepath.Join(dir, "dmpf-reference-reservations"),
	}
	for path, pkg := range map[string]string{
		out.bff:          modulePrefix + "dmpf-reference-bff-go/cmd/dmpf-reference-bff",
		out.orders:       modulePrefix + "dmpf-reference-orders-go/cmd/dmpf-reference-orders",
		out.reservations: modulePrefix + "dmpf-reference-reservations-go/cmd/dmpf-reference-reservations",
	} {

...
| --- | --- | --- |
| `api` | Serve `company.reservations.service.v1.ReservationsService` por gRPC: `Reserve`, `Cancel`, `FindReservation` | `grpc` (servidor, admissão) → `reservationsapp` → `postgres` (UoW, outbox, reader) |
| `relay` | Drena a outbox de `dmpf_reservations` para `reservations.events` | `app/relay` → `postgres` (claim) + `kafka` (publisher) |
| `consumer` | Consome `OrderPlaced` de `orders.events` pela inbox | `kafka` (consumer) → `app` (adapter) → `reservationsapp` → `postgres` (inbox, outbox) |
...
`kernel/reference-reservations-app`, bloco `app`, `bounded_context` `kernel`. É composition root: o único lugar onde os providers concretos de `reservations` são instanciados (ADR-015).
...
Unitários, sem banco: binding e interceptors por `bufconn` sobre o store em memória, ciclo de saúde, tracer de banco, `Sink` (span com pai remoto e filtro por tipo), catálogos por papel e partida do binário. A topologia inteira é provada pelo e2e do `bff`.

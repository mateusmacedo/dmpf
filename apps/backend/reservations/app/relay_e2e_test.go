...
type outboxOnly struct{ Outbox ports.Outbox }

// orderPlacedEvent stands in for the orders context's domain event: this test
// plays the producer through the published contract only, so the orders
// domain and provider stay out of the reservations module.
type orderPlacedEvent struct {
	Order string
	Items int
}

func (orderPlacedEvent) EventName() string { return "orders.order-placed" }

type orderPlacedMapper struct{}

func (orderPlacedMapper) Map(event kernel.DomainEvent) (postgres.Mapped, error) {
	...
	return postgres.Mapped{
		Message: &eventv1.OrderPlaced{OrderId: placed.Order, ItemCount: int32(placed.Items)},
...
func enqueueOrderPlaced(t *testing.T, pool *pgxpool.Pool) {
	...
	uow := postgres.NewUnitOfWork(pool, func(tx *postgres.Tx) outboxOnly {
		return outboxOnly{Outbox: tx.Outbox(orderPlacedMapper{})}
	})
	err := uow.Within(context.Background(), func(ctx context.Context, r outboxOnly) error {
		return r.Outbox.Enqueue(ctx, ports.OutboxEntry{
			MessageID:        relayMessageID,
			OccurredAt:       e2eOccurred,
			Intent:           ports.PublishIntent{Destination: "orders.integration", PartitionKey: relayOrderID},
			AggregateType:    "order",
			AggregateID:      relayOrderID,
			AggregateVersion: 1,
			Event:            orderPlacedEvent{Order: relayOrderID, Items: relayItems},
		})
	})
	if err != nil {
...
func TestAFailureBetweenPublishingAndMarkingRepublishesAndTheInboxAbsorbsIt(t *testing.T) {
	...
	for delivery := 1; delivery <= 2; delivery++ {
		...
		if delivery == 2 && outcome.Disposition != usecase.R2 {

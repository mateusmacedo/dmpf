...
func TestEveryOperationDeclaresItsPermission(t *testing.T) {
	cases := map[string]struct {
		...
		"ConsumeOrderPlaced": {application.ConsumeOrderPlaced{}, "reservations:consume"},
		"Reserve":            {application.Reserve{}, "reservations:write"},
		"Cancel":             {application.Cancel{}, "reservations:write"},
		"FindReservation":    {application.FindReservation{}, "reservations:read"},
	}
...
func TestAuthorizationDeniesASubjectWithoutThePermission(t *testing.T) {
	...
	if err := Authorization()(ctx, application.FindReservation{}); !errors.Is(err, ports.ErrDenied) {

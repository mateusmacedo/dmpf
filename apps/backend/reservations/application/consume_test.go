...
func TestConsumeFirstReceptionAppliesAndConfirms(t *testing.T) {
	...
	disp, err := svc.Consume(context.Background(), testExecution(t), consumeOrderPlaced("m-ext-1", "h1", "P-100", 3))
	...
	snapshot, _, err := reservationsTable.Reader(store).Load(context.Background(), "P-100")
	if err != nil {
...
func TestConsumeCopiesTheMessageContextIntoTheOutboxEntry(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if _, err := svc.Consume(tt.ctx, testExecution(t), consumeOrderPlaced("m-ext-1", "h1", "P-100", 3)); err != nil {
				t.Fatalf("Consume() error = %v, want nil", err)
			}
...
func TestConsumeZeroItemsRejects(t *testing.T) {
	...
	disp, err := svc.Consume(context.Background(), testExecution(t), consumeOrderPlaced("m-ext-1", "h1", "P-100", 0))
	...
	if _, _, err := reservationsTable.Reader(store).Load(context.Background(), "P-100"); !errors.Is(err, ports.ErrNotFound) {
...
func TestConsumeRedeliveryOfAProcessedMessageIsR2(t *testing.T) {
	store := memory.New()
	svc := newService(store)
	if _, err := svc.Consume(context.Background(), testExecution(t), consumeOrderPlaced("m-ext-1", "h1", "P-100", 3)); err != nil {
...
func TestConsumeRedeliveryOfARejectedMessageIsR3(t *testing.T) {
	store := memory.New()
	svc := newService(store)
	if _, err := svc.Consume(context.Background(), testExecution(t), consumeOrderPlaced("m-ext-1", "h1", "P-100", 0)); err != nil {
		t.Fatalf("setup Consume() = %v, want nil", err)
	}
...
func TestConsumeADivergentHashOnAPresentKeyIsR4(t *testing.T) {
	...
	disp, err := svc.Consume(context.Background(), testExecution(t), consumeOrderPlaced("m-ext-1", "h2", "P-100", 3))
...
func TestConsumeTwoMessagesForTheSameOrderReserveOnlyOnce(t *testing.T) {
	...
	disp, err := svc.Consume(context.Background(), testExecution(t), consumeOrderPlaced("m-ext-2", "h2", "P-100", 5))
...
func TestConsumeAuthorizeDenyingNeverOpensATransaction(t *testing.T) {
	store := memory.New()
	svc := newService(store)
	svc.Authorize = func(context.Context, ports.ExecutionContext, application.Operation) error {
		return errors.New("consume_test: not authorized")
	}

	disposition, err := svc.Consume(context.Background(), testExecution(t), consumeOrderPlaced("m-ext-1", "h1", "P-100", 3))
...
func TestConsumeAuthorizeDenyingWithACategoryKeepsIt(t *testing.T) {
	...
	disposition, err := svc.Consume(context.Background(), testExecution(t), consumeOrderPlaced("m-ext-2", "h1", "P-100", 3))
...
func TestConsumeOnACanceledReservationIsR1D2WithoutWriting(t *testing.T) {
	store := memory.New()
	svc := newService(store)
	if _, err := svc.Cancel(context.Background(), testExecution(t), application.Cancel{Order: "o-1"}); err != nil {
		t.Fatalf("setup: Cancel() error = %v", err)
	}
	entriesBefore := len(store.Entries())

	disp, err := svc.Consume(context.Background(), testExecution(t), consumeOrderPlaced("msg-1", "hash-1", "o-1", 2))

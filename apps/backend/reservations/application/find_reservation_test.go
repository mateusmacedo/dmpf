...
func TestFindReservationReadsOutsideTheUnitOfWork(t *testing.T) {
	...
	snapshot, err := h.service.FindReservation(context.Background(), testExecution(t), syncOrder)
...
func TestFindReservationReportsErrNotFoundForAnAbsentAggregate(t *testing.T) {
	...
	_, err := h.service.FindReservation(context.Background(), testExecution(t), "P-404")

...
import (
	...
	servicev1 "github.com/mateusmacedo/dmpf/libs/backend/go/dmpf-contracts/gen/go/company/reservations/service/v1"
...
func TestCancelAfterReserveTravelsAsARejection(t *testing.T) {
	...
	if got := resp.GetRejection(); got.GetCode() != string(reservations.CodeAlreadyReserved) || got.GetMessage() == "" {
		t.Fatalf("rejection = %v, want %q — the first decision won", got, reservations.CodeAlreadyReserved)
	}
}

...
func TestReserveWalksTheNineStepsInOrder(t *testing.T) {
	...
	if _, err := h.service.ReserveBooking(context.Background(), testExecution(t), application.Reserve{
...
func TestCancelWalksTheNineStepsInOrder(t *testing.T) {
	...
	if _, err := h.service.CancelBooking(context.Background(), testExecution(t), application.Cancel{
...
func TestRegisterWalksTheNineStepsInOrder(t *testing.T) {
	...
	if _, err := h.service.RegisterResource(context.Background(), testExecution(t), application.Register{
		Code: testResCode,
	}); err != nil {
...
func TestCancelRejectsWhenBookingNotReserved(t *testing.T) {
	...
	outcome, err := h.service.CancelBooking(context.Background(), testExecution(t), application.Cancel{
		BookingID: testBookingID,
	})
	if err != nil {
		t.Fatalf("CancelBooking() error = %v, want nil", err)
